...
روش اندازه‌گیری: بیلد واقعی با `electron-builder --win --dir` روی پیکربندی جدید → ‏`dist/win-unpacked` برابر **۳۳۱ مگ** (قبلاً ~۸۲۰ مگ) → فشرده‌سازیِ معادل NSIS (LZMA/xz) برابر **۹۷ مگ**. حد بالای بازه (۱۱۰) برای تفاوت‌های جزئی ماشین ویندوز (native buildها، uninstaller و سربار NSIS) منظور شده است.
