...
function browserApi() {
  return {
    ...
    update: {
      check: async () => ({ currentVersion: '2.0.20', latestVersion: '2.0.20', hasUpdate: false, releaseNotes: '', downloaded: false, downloading: false, paused: false, percent: 0, isPackaged: false }),
    ...
    app: { info: async () => ({ version: '2.0.20', platform: 'Browser preview', dataPath: 'Local browser storage' }), openExternal: async (url) => window.open(url, '_blank', 'noopener,noreferrer') }
  }
}
