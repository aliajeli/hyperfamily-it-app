...
function browserApi() {
  return {
    ...
    storeUpdate: (() => {
      ...
      const buses = { version: bus(), step: bus(), progress: bus(), finished: bus() }
      ...
      const demoVersion = (co) => {
        ...
        return { state: 'ok', version: ['3.4.1', '3.4.1', '3.3.8', '3.2.2'][hash % 4], product: 'Store Commerce', source: 'control-panel', publisher: 'Microsoft', pingTime: 3 + (hash % 40) }
      }
      const jalaliToday = () => {
        try {
