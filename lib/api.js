...
function browserApi() {
  return {
    ...
    update: {
      check: async () => ({ currentVersion: '2.0.15', latestVersion: '2.0.15', hasUpdate: false, downloaded: false, downloading: false, percent: 0, isPackaged: false }),
    ...
    app: { info: async () => ({ version: '2.0.15', platform: 'Browser preview', dataPath: 'Local browser storage' }), openExternal: async (url) => window.open(url, '_blank', 'noopener,noreferrer') }
  }
}
