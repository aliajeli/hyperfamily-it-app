...
function initialState() {
  return {
    ...
    notes: [
      { id: 1, name: 'Branch rollout checklist', body: '1. Rack and label the switch\n2. Uplink to the router on Gi1/0/24\n3. Register the asset code in Inventory\n4. Verify ping from the dashboard', pinned: 1, color: 'blue', priority: 1, created_at: new Date().toISOString(), updated_at: new Date().toISOString() },
      { id: 2, name: 'VLAN plan', body: 'VLAN 10 - Staff\nVLAN 20 - POS\nVLAN 30 - Cameras\nVLAN 99 - Management', pinned: 0, color: 'default', priority: 0, created_at: new Date().toISOString(), updated_at: new Date().toISOString() }
    ],
    snippets: [
...
function browserApi() {
  return {
    ...
    update: {
      check: async () => ({ currentVersion: '2.0.14', latestVersion: '2.0.14', hasUpdate: false, downloaded: false, downloading: false, percent: 0, isPackaged: false }),
      state: async () => ({ downloading: false, downloaded: false, percent: 0, version: null, canInstall: false, isPackaged: false }),
    ...
    app: { info: async () => ({ version: '2.0.14', platform: 'Browser preview', dataPath: 'Local browser storage' }), openExternal: async (url) => window.open(url, '_blank', 'noopener,noreferrer') }
  }
}
