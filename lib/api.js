  ...
  return {
    ...
    storeUpdate: (() => {
      ...
      const demoVersion = (co) => {
        ...
        if (hash % 13 === 0) return { state: 'not-found', extension: hash % 2 === 0 ? { version: '1.0.33', source: 'file' } : null }
        return { state: 'ok', version: ['3.4.1', '3.4.1', '3.3.8', '3.2.2'][hash % 4], product: 'Store Commerce', source: 'agent', publisher: 'Microsoft', pingTime: 3 + (hash % 40), extension: hash % 7 === 0 ? null : { version: `1.0.${20 + (hash % 30)}`, source: hash % 5 === 0 ? 'file' : 'control-panel' } }
      }
      const jalaliToday = () => {
        try {
