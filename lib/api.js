...
function browserApi() {
  return {
    ...
    update: {
      check: async () => ({ currentVersion: '2.0.22', latestVersion: '2.0.22', hasUpdate: false, releaseNotes: '', downloaded: false, downloading: false, paused: false, percent: 0, isPackaged: false }),
    ...
    software: {
      listInstalled: async () => [
        { name: 'FortiClient VPN', version: '7.2.4.0972', publisher: 'Fortinet Inc', installLocation: 'C:\\Program Files\\Fortinet\\FortiClient\\', displayIcon: 'C:\\Program Files\\Fortinet\\FortiClient\\FortiClient.exe' },
        { name: 'TeamViewer 15', version: '15.58.5', publisher: 'TeamViewer Germany GmbH', installLocation: 'C:\\Program Files\\TeamViewer\\', displayIcon: 'C:\\Program Files\\TeamViewer\\TeamViewer.exe' },
        { name: '7-Zip 24.05 (x64)', version: '24.05', publisher: 'Igor Pavlov', installLocation: 'C:\\Program Files\\7-Zip\\', displayIcon: 'C:\\Program Files\\7-Zip\\7zFM.exe' },
        { name: 'Node.js', version: '22.12.0', publisher: 'Node.js Foundation', installLocation: 'C:\\Program Files\\nodejs\\', displayIcon: '' },
        { name: 'Git', version: '2.46.2', publisher: 'The Git Development Community', installLocation: 'C:\\Program Files\\Git\\', displayIcon: '' }
      ],
      checkVersion: async (payload) => {
        if (String(payload?.path || '').trim()) {
          const filePath = String(payload.path).trim()
          return { mode: 'file', path: filePath, fileName: filePath.split(/[\\/]/).pop(), fileVersion: '2.0.22.0', productVersion: '2.0.22', productName: 'Demo executable', companyName: 'Browser preview', fileDescription: '', sizeBytes: 184_000_000, modifiedAt: new Date().toISOString() }
        }
        const needle = String(payload?.name || '').trim().toLowerCase()
        if (!needle) throw new Error('Enter a program name or choose an executable file')
        const programs = await getApi().software.listInstalled()
        const matches = programs.filter((program) => program.name.toLowerCase().includes(needle))
        return { mode: 'installed', query: payload.name, matches, total: matches.length, checkedAt: new Date().toISOString() }
      },
      copyFiles: async () => { throw new Error('File copying runs in the Windows desktop app') },
      onCopyProgress: () => () => {}
    },
    audit: { list: async () => readState().audit.slice(0, 100) },
    dialog: { selectFile: async () => null, selectFiles: async () => [], selectDirectory: async () => null },
    app: { info: async () => ({ version: '2.0.22', platform: 'Browser preview', dataPath: 'Local browser storage' }), openExternal: async (url) => window.open(url, '_blank', 'noopener,noreferrer') }
  }
}
