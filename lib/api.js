    ...
    branches: {
      ...
      save: async (data) => withState((s) => {
        const normalized = data.id
          ? { ...s.branches.find((item) => item.id === Number(data.id)), ...data, id: Number(data.id) }
          : { ...data, id: Math.max(0, ...s.branches.map((item) => item.id)) + 1, created_at: new Date().toISOString() }
    ...
    inventory: {
      list: async () => {
        ...
        return buildSnapshot().devices.map((device) => {
          ...
          return { ...device, branch_name: branch?.name || '—', branch_code: branch?.code || '—' }
        })
      },
