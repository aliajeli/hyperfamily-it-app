...
function browserApi() {
  return {
    ...
    directory: {
      ...
      importFromHost: async ({ host } = {}) => {
        if (!String(host || '').trim()) throw new Error('Enter the IP address or hostname of the source workstation')
        // Simulated network round-trip for the browser demo build.
        await new Promise((resolve) => setTimeout(resolve, 2200))
        return { host: String(host).trim(), branches_added: 2, branches_updated: 1, devices_added: 9, devices_updated: 4, switch_ports_imported: 36 }
      }
    },
    remote: {
    ...
    storeUpdate: (() => {
      ...
      const buses = { version: bus(), step: bus(), progress: bus(), finished: bus(), agentStep: bus(), installStep: bus(), installFinished: bus() }
      ...
      const demoVersion = (co) => {
        ...
        if (hash % 13 === 0) return { state: 'not-found', extension: hash % 2 === 0 ? { name: 'Hyper.Commerce', version: '2.18.4', source: 'manifest' } : null }
        return { state: 'ok', version: ['3.4.1', '3.4.1', '3.3.8', '3.2.2'][hash % 4], product: 'Store Commerce', source: 'agent', publisher: 'Microsoft', pingTime: 3 + (hash % 40), extension: hash % 7 === 0 ? null : { name: 'Hyper.Commerce', version: `2.19.${10 + (hash % 20)}`, source: 'manifest' } }
      }
      const jalaliToday = () => {
        try {
      ...
      async function demoImportAgent({ checkout, runId }) {
        ...
        const emit = (step, status, detail, progress) => buses.agentStep.emit({ checkoutId: checkout.id, name: checkout.name, step, status, detail, ...(progress ? { progress } : {}) })
        const cancelledError = () => Object.assign(new Error('Import stopped by the operator'), { code: 'AGENT_IMPORT_CANCELLED', cancelled: true })
        const stopped = () => { if (run.cancelled) throw cancelledError() }
        const startedAt = Date.now()
        try {
          stopped()
          ...
          emit('compare', 'running', copied ? 'Agent EXE is missing — copy required' : 'Reading installed agent SHA-256 over SMB')
          await sleep(360)
          emit('compare', 'done', copied ? 'Agent EXE is missing — copy required' : 'SHA-256 matches — copy skipped')
          stopped()
          if (copied) {
            ...
            for (let percent = 0; percent <= 100; percent += 20) {
              stopped()
              emit('copy', 'running', `Copying staged agent: ${((totalBytes * percent) / 1000000).toFixed(1)} / 2.4 MB (${percent}%), 1840 KB/s`, { bytes: Math.round(totalBytes * percent / 100), totalBytes, elapsedMs: percent * 12, bytesPerSecond: 1840000 })
              await sleep(130)
            }
          ...
          emit('finish', 'done', copied ? 'Copied, verified and the service is Running' : 'SHA-256 matched — the service is Running')
          importedAgents.add(checkout.id)
          return { checkoutId: checkout.id, name: checkout.name, ok: true, copied, state: 'running', durationMs: Date.now() - startedAt }
        } catch (error) {
          if (run.cancelled) {
            ...
            return { checkoutId: checkout.id, name: checkout.name, ok: false, cancelled: true, error: 'Import stopped by the operator', durationMs: Date.now() - startedAt }
          }
          emit('failed', 'done', error.message)
          return { checkoutId: checkout.id, name: checkout.name, ok: false, error: error.message, durationMs: Date.now() - startedAt }
        } finally {
      ...
      async function demoInstallOne({ checkout, destinationPath, runId }) {
        ...
        const emit = (step, status, detail) => {
          ...
          buses.installStep.emit({ runId: id, checkoutId: checkout.id, name: checkout.name, step, status, detail, at: new Date().toISOString() })
        }
        const stopped = () => { if (run.cancelled) throw Object.assign(new Error('Stopped by the operator'), { cancelled: true }) }
        const finish = (ok, extra = {}) => ({ checkoutId: checkout.id, name: checkout.name, host: checkout.ip || checkout.hostname, ok, steps, durationMs: Date.now() - startedAt, ...extra })
        ...
        try {
          ...
          await sleep(450)
          if (base.state === 'offline') { emit('reachability', 'failed', 'SMB (port 445) did not answer'); return finish(false, { error: 'Checkout unreachable' }) }
          ...
          await sleep(420)
          if (!importedAgents.has(checkout.id)) { emit('agent', 'failed', 'Agent is not running'); return finish(false, { error: 'Agent is not running — use Import Agent first' }) }
          ...
          stopped()
          emit('file-check', 'running', `Looking for Hyper.StoreCommerce.Installer.exe in ${destinationPath}…`)
          await sleep(380)
          if (hash % 13 === 3) {
            emit('file-check', 'failed', `Hyper.StoreCommerce.Installer.exe was not found in ${destinationPath}`)
            emit('version', 'done', `Store Commerce v${versionBefore}`)
            return finish(false, { error: 'Hyper.StoreCommerce.Installer.exe not found — deploy it first', version: versionBefore, versionBefore })
          }
          emit('file-check', 'done', `Hyper.StoreCommerce.Installer.exe (4812032 bytes) is in ${destinationPath}`)
          stopped()
          ...
          if (failedInstall) {
            ...
            emit('version', 'done', `Store Commerce v${versionBefore}`)
            return finish(false, { error: 'Installer exit code 1603', exitCode: 1603, output: 'Setup encountered a fatal error 1603 while installing Store Commerce.\nRolling back the installation… done.', timedOut: false, version: versionBefore, versionBefore })
          }
          ...
          const result = finish(true, { exitCode: 0, output: 'Preparing installation…\nInstalling Store Commerce 9.60.24100.1…\nApplying configuration…\nInstallation completed successfully.', version: versionAfter, versionBefore })
          demoInstallState.set(checkout.id, result)
          return result
        } catch (error) {
          if (error.cancelled) { emit('cancelled', 'skipped', 'Stopped by the operator'); return finish(false, { cancelled: true, error: error.message }) }
          ...
          try {
            for (const checkout of checkouts) {
              if (demoAgentRuns.get(id)?.cancelled) {
                results.push({ checkoutId: checkout.id, name: checkout.name, ok: false, cancelled: true, skipped: true, error: 'Skipped — the import was stopped' })
                continue
              }
              results.push(await demoImportAgent({ checkout, runId: id }))
            }
            cancelledByOperator = Boolean(demoAgentRuns.get(id)?.cancelled)
          } finally { demoAgentRuns.delete(id) }
          return {
            runId: id,
          ...
          try {
            for (const checkout of checkouts) {
              if (demoInstallRuns.get(id)?.cancelled) {
                ...
                buses.installStep.emit({ runId: id, checkoutId: checkout.id, name: checkout.name, step: 'cancelled', status: 'skipped', detail: 'Skipped after the operator stopped the batch', at: new Date().toISOString() })
                continue
              }
              results.push(await demoInstallOne({ checkout, destinationPath, runId: id }))
            }
          } finally { demoInstallRuns.delete(id) }
          const summary = {
            runId: id,
            total: results.length,
            ok: results.filter((row) => row.ok).length,
