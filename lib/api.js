...
function browserApi() {
  return {
    ...
    update: {
      check: async () => ({ currentVersion: '2.0.2', latestVersion: '2.0.2', hasUpdate: false }),
    ...
    app: { info: async () => ({ version: '2.0.2', platform: 'Browser preview', dataPath: 'Local browser storage' }), openExternal: async (url) => window.open(url, '_blank', 'noopener,noreferrer') }
  }
}
