...
function browserApi() {
  return {
    ...
    remote: {
      ...
      guacamole: async () => { throw new Error('Remote sessions can only open from the Windows desktop app') },
      guacamoleTest: async () => { throw new Error('Guacamole can only be reached from the Windows desktop app') },
      guacamoleClose: async () => ({ success: true })
    },
    vpn: {
    ...
    update: {
      check: async () => ({ currentVersion: '2.0.0', latestVersion: '2.0.0', hasUpdate: false }),
    ...
    app: { info: async () => ({ version: '2.0.0', platform: 'Browser preview', dataPath: 'Local browser storage' }), openExternal: async (url) => window.open(url, '_blank', 'noopener,noreferrer') }
  }
}
