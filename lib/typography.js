 ...
 *
 * Storage stays on the 50–200 % grid so older settings keep working, but the
 * Fonts & scale tab speaks in the pixels the user actually sees. pxForScale is
...
export function pxForScale(groupId, scale) {
  ...
  return Math.round((base * normalizeScale(scale)) / 100)
}

export function scaleForPx(groupId, px) {
  const base = FONT_GROUP_BASE_PX[groupId] || 16
  return normalizeScale((Number(px) / base) * 100)
}

/** Whole-pixel options a group offers, from 50 % to 200 % of its base size. */
export function pxOptionsFor(groupId) {
  const base = FONT_GROUP_BASE_PX[groupId] || 16
  const min = Math.max(6, Math.round((base * SCALE_MIN) / 100))
  const max = Math.round((base * SCALE_MAX) / 100)
  const options = []
