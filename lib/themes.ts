...
export const THEMES = [
  ...
  theme(
    ...
    'dark',
    'Nord',
    'Deep Polar Night surfaces with strong blue Frost accents',
    {
      primary: '129 161 193',
      secondary: '136 192 208',
      accent: '143 188 187',
  ...
  theme(
    ...
    'dark',
    'Nord',
    'Dark Nord surfaces with purple and red Aurora accents',
    {
      primary: '180 142 173',
  ...
  theme(
    ...
    'light',
    'Modern',
    'Neutral slate surfaces with a confident indigo primary',
    {
      primary: '79 70 229',
  ...
  theme(
    ...
    'light',
    'Modern',
    'Monochrome workspace with a single amber highlight',
    {
      primary: '55 65 81',
  ...
  theme(
    ...
    'dark',
    'Modern',
    'Deep navy surfaces with an electric indigo primary',
    {
      primary: '129 140 248',
      secondary: '99 102 241',
      accent: '56 189 248',
  ...
  theme(
    ...
    'dark',
    'Nord',
    'Polar Night pushed darker with icy Frost edges',
    {
      primary: '136 192 208',
      secondary: '129 161 193',
      accent: '236 239 244',
...
/* Solid fills that carry white text need a guaranteed-readable shade: several
   Nord accents are too bright for that pairing (white on aurora red reaches
   only 4.1:1, below the 4.5:1 WCAG minimum). readableOnWhite() scales a hue
   down — same family, slightly deeper — just far enough to clear 4.6:1, and
   applyTheme() derives --primary-strong / --danger-strong from it so catalog
   and custom palettes both get an accessible button color. Decorative uses
   (dots, bars, borders) keep the untouched, brighter --primary. */
...
export function readableOnWhite(triplet) {
  const rgb = String(triplet)
    .trim()
    .split(/[\s,]+/)
    .map(Number)
  let scale = 1
  while (
    1.05 / (relativeLuminance(rgb.map((c) => c * scale) as [number, number, number]) + 0.05) < 4.6 &&
    scale > 0.2
  )
    scale -= 0.01
  return rgb.map((c) => Math.max(0, Math.min(255, Math.round(c * scale)))).join(' ')
}
/** Accessible stand-in for nord-11, which the danger buttons used raw. */
export const DANGER_STRONG = readableOnWhite('191 97 106')
...
function isLight(canvasTriplet) {
  const [r, g, b] = String(canvasTriplet || '0 0 0')
    .split(/\s+/)
    .map(Number)
  const channel = (value) => {
 ...
 */
export function findTheme(id: any, customColors?: any) {
...
export function applyTheme(themeOrId: any, customColors?: any) {
  ...
  // Derived accessibility shades for solid fills with white text.
  root.style.setProperty('--primary-strong', readableOnWhite(value.primary))
  root.style.setProperty('--danger-strong', DANGER_STRONG)
  ...
  try {
    ...
    /* auxiliary theming is best-effort */
  }
}
 ...
 */
export function applyThemeAnimated(themeOrId: any, customColors?: any, _origin?: any) {
  ...
  if (
    ...
    !windowObject ||
    reducedMotion ||
    !documentObject.startViewTransition
  ) {
