...
if (typeof window !== 'undefined') {
  ...
  } catch { /* very old runtimes */ }
}

/**
 * Applies a theme with a polished, commercial-grade transition (v2.0.15).
 *
 * The effect paints in the page BACKGROUND — every element stays fully
 * visible the whole time. Three things run together:
 *
 *   1. Iris — a soft radial disc in the new theme's canvas colour grows out
 *      of the pointer position across the background (behind the interface,
 *      z-index -1, ~0.8s gentle ease-out).
 *   2. Shine — a diagonal band of light sweeps the screen once, also behind
 *      the elements, catching the translucent panels.
 *   3. Crossfade — the moment the iris has covered the background, the theme
 *      is applied and every element transitions its colours in one smooth
 *      ~0.75s crossfade (`html.theme-crossfade` in globals.css).
 *
 * Falls back to a plain apply when the runtime lacks Web Animations, when the
 * user prefers reduced motion, or when a transition is already in flight.
 */
export function applyThemeAnimated(themeOrId, customColors, origin = null) {
  ...
  if (themeTransitionRunning || !documentObject || !windowObject || reducedMotion) {
    applyTheme(themeOrId, customColors)
    return
  }

  const value = typeof themeOrId === 'string' ? findTheme(themeOrId, customColors) : themeOrId
  const start = origin || lastPointer || { x: windowObject.innerWidth / 2, y: windowObject.innerHeight / 2 }

  // Radius = furthest corner from the origin, so the circle can always cover.
  const corners = [
    [0, 0], [windowObject.innerWidth, 0],
    [0, windowObject.innerHeight], [windowObject.innerWidth, windowObject.innerHeight]
  ]
  const radius = Math.ceil(Math.max(...corners.map(
    ([cx, cy]) => Math.hypot(cx - start.x, cy - start.y))) + 24)
  const size = radius * 2

  // Iris: paints in the background — z-index -1 puts it above the page's own
  // canvas colour but beneath every element, so nothing is ever covered.
  const iris = documentObject.createElement('div')
  Object.assign(iris.style, {
    ...
    height: `${size}px`,
    borderRadius: '50%',
    background: `radial-gradient(circle at 50% 50%, rgb(${value.surface}) 0%, rgb(${value.canvas}) 62%)`,
    boxShadow: `inset 0 0 0 1px rgb(${value.primary} / .35), inset 0 0 140px rgb(0 0 0 / .05)`,
    pointerEvents: 'none',
    zIndex: '-1',
    transform: 'scale(0)',
    opacity: '1',
  ...
  documentObject.body.appendChild(iris)

  // Shine: the light sweep, also behind the interface.
  const shine = documentObject.createElement('div')
  Object.assign(shine.style, {
    position: 'fixed',
    inset: '0',
    background: 'linear-gradient(115deg, transparent 30%, rgba(255,255,255,.22) 45%, rgba(255,255,255,.45) 50%, rgba(255,255,255,.22) 55%, transparent 70%)',
    pointerEvents: 'none',
    zIndex: '-1',
    opacity: '0',
    willChange: 'transform, opacity'
  })
  documentObject.body.appendChild(shine)

  themeTransitionRunning = true
  const cleanup = () => {
    themeTransitionRunning = false
    documentObject.documentElement.classList.remove('theme-crossfade')
    try { iris.remove() } catch { /* already gone */ }
    try { shine.remove() } catch { /* already gone */ }
  }
  const finish = () => {
    // Background covered: repaint the theme and crossfade every element's
    // colours so the whole interface glides into the new palette.
    documentObject.documentElement.classList.add('theme-crossfade')
    applyTheme(value, customColors)
    const fade = iris.animate(
      [{ opacity: 1 }, { opacity: 0 }],
      { duration: 620, easing: 'ease-out', fill: 'forwards' }
    )
    fade.addEventListener('finish', () => {
      windowObject.setTimeout(cleanup, 160)
    })
    fade.addEventListener('cancel', cleanup)
    // Safety net: never leave the crossfade class behind.
    windowObject.setTimeout(() => documentObject.documentElement.classList.remove('theme-crossfade'), 1100)
  }

  try {
    const grow = iris.animate(
      [{ transform: 'scale(0)' }, { transform: 'scale(1)' }],
      { duration: 800, easing: 'cubic-bezier(0.22, 1, 0.36, 1)', fill: 'forwards' }
    )
    grow.addEventListener('finish', finish)
    grow.addEventListener('cancel', () => { applyTheme(value, customColors); cleanup() })

    shine.animate(
      [
        { opacity: 0, transform: 'translateX(-120%)' },
        { opacity: 1, transform: 'translateX(-8%)', offset: 0.18 },
        { opacity: 1, transform: 'translateX(8%)', offset: 0.82 },
        { opacity: 0, transform: 'translateX(120%)' }
      ],
      { duration: 950, delay: 100, easing: 'cubic-bezier(0.45, 0, 0.55, 1)', fill: 'forwards' }
    )
  } catch {
    applyTheme(value, customColors)
    cleanup()
