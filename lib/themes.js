...
/**
 * Applies a theme with a soft "re-paint" animation that starts exactly where
 * the pointer is (v2.0.13).
 *
 * A circle in the new theme's canvas colour grows out of the pointer position
 * and covers the screen; the real theme is applied the instant it is fully
 * covered (so nothing visibly snaps), and the circle then fades away to reveal
 * the freshly painted interface. Duration and easing are deliberately gentle.
 *
...
export function applyThemeAnimated(themeOrId, customColors, origin = null) {
  ...
  try {
    const grow = overlay.animate(
      ...
      { duration: 820, easing: 'cubic-bezier(0.33, 1, 0.68, 1)', fill: 'forwards' }
    )
    grow.addEventListener('finish', () => {
      // Covered: repaint underneath, then let the old palette fade away.
