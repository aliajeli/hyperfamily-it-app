...
/**
 * Applies a theme with one clean, coordinated fade (v2.0.16).
 *
 * No overlays, no ripples, no extra effects: the palette is applied
 * immediately and every element of the interface crossfades its colours into
 * the new theme in a single ~0.65s fade — headers, cards, text and borders
 * all change together, exactly like a fade transition in a video.
 *
 * The crossfade itself lives in globals.css (html.theme-crossfade). Reduced
 * motion and concurrent calls fall back to an instant apply.
 */
export function applyThemeAnimated(themeOrId, customColors) {
  ...
  if (!documentObject || reducedMotion || themeTransitionRunning) {
