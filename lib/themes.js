...
/**
 * Applies a theme with a polished, commercial-grade transition (v2.0.14).
 *
 * Two layers choreographed together:
 *
 *   1. Iris — a soft radial disc in the new theme's canvas colour grows out of
 *      the pointer position and covers the screen (~0.8s, gentle ease-out).
 *      The real theme is applied the instant it is fully covered, so nothing
 *      ever visibly snaps.
 *   2. Shine — a diagonal band of light sweeps across the screen while the
 *      iris grows, like light passing over glass. A primary-coloured hairline
 *      ring sits on the iris edge so the whole effect reads as a deliberate
 *      brand moment rather than a loading spinner.
 *
 * The iris then fades away (~0.5s) to reveal the freshly painted interface.
 * Total ≈ 1.3s. Falls back to a plain apply when the runtime lacks Web
 * Animations, when the user prefers reduced motion, or when a transition is
 * already in flight.
 */
export function applyThemeAnimated(themeOrId, customColors, origin = null) {
  ...
  if (themeTransitionRunning || !documentObject || reducedMotion) {
  ...
  // Iris: a soft disc that starts where the pointer is.
  ...
  // Shine: a diagonal band of light that sweeps the screen once.
