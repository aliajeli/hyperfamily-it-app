...
function normalizeSwitchPorts(ports, deviceId) {
  ...
  if (ports.length > MAX_SWITCH_PORTS)
    throw new Error(`A Switch can contain at most ${MAX_SWITCH_PORTS} ports`)
  const seen = new Set()
...
const seedBranches = [
  {
    ...
    name: 'Central Berlin',
    code: 'BER-01',
    warehouse_code: 'WH-BER-01',
    manager_name: 'Sarah Klein',
    manager_tell: '+49 30 555 0101',
    deputy_name: 'Martin Vogel',
    deputy_tell: '+49 30 555 0102',
    link1: 'MPLS Primary',
    ip_link1: '10.10.1.1',
    link2: 'LTE Backup',
    ip_link2: '10.10.1.2'
  },
  {
    ...
    name: 'Alexanderplatz',
    code: 'BER-02',
    warehouse_code: 'WH-BER-02',
    manager_name: 'Daniel Weber',
    manager_tell: '+49 30 555 0201',
    deputy_name: 'Emma Roth',
    deputy_tell: '+49 30 555 0202',
    link1: 'Fiber Primary',
    ip_link1: '10.20.1.1'
  },
  {
    ...
    name: 'Potsdam',
    code: 'POT-01',
    warehouse_code: 'WH-POT-01',
    manager_name: 'Lena Fischer',
    manager_tell: '+49 331 555 0301',
    deputy_name: 'Noah Wolf',
    deputy_tell: '+49 331 555 0302',
    link1: 'MPLS Primary',
    ip_link1: '10.30.1.1'
  },
  {
    ...
    name: 'Spandau',
    code: 'BER-03',
    warehouse_code: 'WH-BER-03',
    manager_name: 'Mia Wagner',
    manager_tell: '+49 30 555 0401',
    deputy_name: 'Leon Braun',
    deputy_tell: '+49 30 555 0402',
    link1: 'Fiber Primary',
    ip_link1: '10.40.1.1'
  }
]
...
const seedDevices = seedBranches.flatMap((branch, branchIndex) => {
  const templates = [
    {
      device_type: 'Router',
      name: `${branch.code} Gateway`,
      model: 'MikroTik CCR2004',
      location: 'Network room',
      connection_type: 'Winbox'
    },
...
function initialState() {
  return {
    ...
    notes: [
      {
        id: 1,
        name: 'Branch rollout checklist',
        body: '1. Rack and label the switch\n2. Uplink to the router on Gi1/0/24\n3. Register the asset code in Inventory\n4. Verify ping from the dashboard',
        pinned: 1,
        color: 'blue',
        priority: 1,
        tags: ['network', 'rollout'],
        created_at: new Date().toISOString(),
    ...
    snippets: [
      ...
      },
      {
        ...
        description: 'Configured VLANs and member ports'
      },
      {
        id: 3,
        name: 'Show MAC table',
        command: 'show mac address-table',
        description: 'Learned MAC addresses'
      },
      {
        id: 4,
      ...
      },
      {
        ...
        description: 'Persist the running configuration'
      }
    ],
    audit: [
      {
...
function normalizeBrowserState(value) {
  ...
  merged.branches = (Array.isArray(merged.branches) ? merged.branches : baseline.branches).map(
    (branch, index) => {
...
async function updateBrowserCredentials(payload: any = {}) {
  ...
  if (newUsername.length < 3 || newUsername.length > 64)
    throw new Error('Username must contain between 3 and 64 characters')
  if (newPassword && newPassword.length < 4)
    throw new Error('New password must contain at least 4 characters')
  ...
  try {
    ...
    if (remembered.username) {
      localStorage.setItem(
        REMEMBER_KEY,
        JSON.stringify({ username: newUsername, password: newPassword || remembered.password })
      )
    }
  } catch {
...
function buildSnapshot() {
  ...
  const devices = state.devices.map((device) => {
    ...
    const ping_time = isOffline
      ? null
      : isSlow
        ? Math.round(320 + cycle * 120)
        : Math.max(2, Math.round(12 + cycle * 115))
    ...
    const history = Array.from({ length: historyCount }, (_, i) => {
      ...
      const value = warning
        ? Math.round(315 + Math.abs(Math.sin(i * 0.31 + device.id)) * 95)
        : Math.round(15 + Math.abs(Math.sin(i * 0.37 + device.id)) * 85)
      return {
        sequence: i + 1,
        ping_time: offline ? null : value,
        status: offline ? 'offline' : statusFromPing(value),
        timestamp: new Date(
          now - (historyCount - i - 1) * Number(state.settings.ping_interval || 3) * 1000
        ).toISOString()
      }
    })
  ...
  const respond = (session, command) => {
    ...
    if (/^(exit|quit|logout)$/i.test(trimmed)) {
      setTimeout(() => {
        ...
        emit('status', { sessionId: session.sessionId, state: 'closed', message: 'Session closed' })
      }, 120)
    ...
    if (/^show\s+mac/i.test(trimmed)) {
      return [
        ...
        '  30    5c02.1234.9900    DYNAMIC     Gi1/0/3',
        ''
      ].join('\r\n')
    }
    if (/^show\s+running-config/i.test(trimmed)) {
      return [
        ...
        '',
        `hostname ${session.name.replace(/\s+/g, '-')}`,
        '!',
        'vlan 10',
        ' name Staff',
        '!',
    ...
    if (/^\?$|^help$/i.test(trimmed))
      return 'Demo commands: show interfaces status, show vlan brief, show mac address-table, show running-config, show version, write memory, exit\r\n'
  ...
  return {
    targets: async () => {
      ...
      return state.branches
        .map((branch) => ({
          ...
          name: branch.name,
          code: branch.code,
          switches: state.devices
            .filter((device) => device.branch_id === branch.id && device.device_type === 'Switch')
            .map((device) => ({
              id: device.id,
              name: device.name,
              ip: device.ip,
              ...
              transport: device.transport === 'telnet' ? 'telnet' : 'ssh'
            }))
        }))
    ...
    open: async ({ deviceId }) => {
      ...
      setTimeout(() => {
        ...
        push(
          session.sessionId,
          'Browser preview \u2014 emulated switch CLI. Type ? for the command list.\r\n\r\n'
        )
...
function browserApi() {
  return {
    ...
    auth: {
      ...
      login: async ({ username, password }) => {
        ...
        if (
          String(username || '')
            .trim()
            .toLowerCase() !== account.username.toLowerCase() ||
          !passwordMatches
        )
      ...
      recover: async (pin) => {
        ...
        if (!account.recoveryPinHash)
          throw new Error('No recovery PIN has been set. Sign in and set one in Settings → General.')
        ...
        if (!matches) {
          ...
          localStorage.setItem(
            'hyperfamily.browser.recovery-lock',
            JSON.stringify({ attempts: lock ? 0 : attempts, lockedUntil: lock ? now + LOCK_MS : 0 })
          )
          return {
            ok: false,
            locked: lock,
            retryAfterMs: lock ? LOCK_MS : 0,
            remainingAttempts: lock ? 0 : MAX_ATTEMPTS - attempts
          }
        }
        localStorage.setItem(
          'hyperfamily.browser.recovery-lock',
      ...
      rememberCredentials: async ({ username, password }: any = {}) => {
    ...
    branches: {
      ...
      save: async (data) =>
        withState((s) => {
          ...
          if (!String(data.name || '').trim() || !code || !warehouseCode)
            throw new Error('Branch Name, Code, and Warehouse Code are required')
          if (!/^[A-Za-z0-9_-]+$/.test(code) || code.length > 20)
            throw new Error('Branch Code must use no more than 20 letters, numbers, dashes, or underscores')
          if (!/^[A-Za-z0-9_-]+$/.test(warehouseCode) || warehouseCode.length > 40)
            throw new Error(
              'Warehouse Code must use no more than 40 letters, numbers, dashes, or underscores'
            )
          const duplicate = s.branches.find(
            (item) =>
              item.id !== Number(data.id) &&
              (item.code?.toLowerCase() === code.toLowerCase() ||
                item.warehouse_code?.toLowerCase() === warehouseCode.toLowerCase())
          )
          ...
          const normalized = data.id
            ...
            : {
                ...cleanData,
                id: Math.max(0, ...s.branches.map((item) => item.id)) + 1,
                created_at: new Date().toISOString()
              }
      ...
      remove: async (id) =>
        withState((s) => {
          ...
          s.devices = s.devices.filter((x) => x.branch_id !== id)
          return { success: true }
        }),
      removeAll: async () =>
        withState((s) => {
    ...
    devices: {
      list: async () =>
        readState().devices.map((device) => ({ ...device, switch_ports: device.switch_ports || [] })),
      save: async (data) =>
        withState((s) => {
          ...
          if (
            ...
            s.devices.some(
              (item) =>
                item.branch_id === Number(data.branch_id) &&
                item.device_type === 'Router' &&
                item.id !== Number(data.id)
            )
          )
            throw new Error('Only one Router can be defined for each branch')
          const deviceId = data.id ? Number(data.id) : Math.max(0, ...s.devices.map((item) => item.id)) + 1
          const switchPorts =
            data.device_type === 'Switch' ? normalizeSwitchPorts(data.switch_ports || [], deviceId) : []
          const normalized = {
            ...data,
            id: deviceId,
          ...
          if (data.id)
            s.devices = s.devices.map((item) =>
              item.id === deviceId ? { ...item, ...normalized, updated_at: new Date().toISOString() } : item
            )
    ...
    // The companion server only exists inside the desktop application; the
    // browser preview reports it as unsupported so the Settings card renders
    // its explanatory state instead of failing.
    companion: {
      state: async () => ({
        running: false,
        enabled: false,
    ...
    credentials: {
      list: async () =>
        readState().credentials.map((x) => ({ ...x, password: undefined, has_password: true })),
      reveal: async (id) => readState().credentials.find((x) => x.id === id)?.password || '',
      save: async (data) =>
        withState((s) => {
          s.credentials.push({ ...data, id: Date.now() })
          return data
        }),
      remove: async (id) =>
        withState((s) => {
      ...
      saveMappings: async (mappings) =>
        withState((s) => {
          s.mappings = normalizeMappings(mappings)
          return s.mappings
        }),
      overview: async () => {
        const s = readState()
        ...
        return s.devices
          ...
          .sort(
            (a, b) => a.branch_name.localeCompare(b.branch_name) || a.device_name.localeCompare(b.device_name)
          )
      },
      assignDevice: async (deviceId, credentialId) =>
        withState((s) => {
          const map = normalizeMappings(s.mappings)
          if (credentialId === null || credentialId === undefined || credentialId === '')
            delete map.devices[deviceId]
          else map.devices[deviceId] = [Number(credentialId)]
          s.mappings = map
          return { device_id: Number(deviceId), credential_id: credentialId ?? null }
        }),
      assignType: async (deviceType, credentialId) =>
        withState((s) => {
          const map = normalizeMappings(s.mappings)
          if (credentialId === null || credentialId === undefined || credentialId === '')
            delete map.types[deviceType]
      ...
      list: async () => {
        ...
        return buildSnapshot().devices.map((device) => {
          ...
          return {
            ...
            branch_warehouse_code: branch?.warehouse_code || '—'
          }
        })
      },
      export: async () => {
        throw new Error('Excel export is available in the Windows desktop app')
      }
    },
    directory: {
      template: async () => {
        throw new Error('The import template can only be generated from the Windows desktop app')
      },
      import: async () => {
        throw new Error('Excel import is available in the Windows desktop app')
      },
      export: async () => {
        throw new Error('Excel export is available in the Windows desktop app')
      }
    ...
    remote: {
      connect: async () => {
        throw new Error('Remote tools can only launch from the Windows desktop app')
      },
      ...
      save: async (payload) => {
        ...
        if (payload.id)
          state.snippets = state.snippets.map((item) =>
            item.id === payload.id ? { ...item, ...record } : item
          )
    ...
    vpn: {
      ...
      probe: async () => ({
        installed: false,
        path: null,
        downloadUrl: 'https://www.fortinet.com/support/product-downloads#vpn',
        configured: false
      }),
      connect: async () => {
        throw new Error('VPN control is available in the Windows desktop app')
      },
      ...
      diagnose: async () => ({
        ...
        stage: 'profile',
        outcome: 'error',
        reason: 'VPN diagnostics are available in the Windows desktop app',
        durationMs: 0
      }),
      subscribe: () => () => {}
    },
    ...
    storeUpdate: (() => {
      ...
      const bus = () => {
        const subs = new Set<(payload) => void>()
        return {
          emit: (payload) =>
            subs.forEach((cb) => {
              try {
      ...
      // Mirrors the main-process version cache: filled by sweeps/rechecks, so
      // reopening the page in the demo shows cached answers too.
      const demoVersionCache = {}
      ...
      const demoVersion = (co) => {
        ...
        if (!importedAgents.has(co.id))
          return { state: 'agent-not-running', detail: 'Demo: use Import Agent' }
        if (hash % 13 === 0)
          return {
            state: 'not-found',
            extension:
              hash % 2 === 0 ? { name: 'Hyper.Commerce', version: '2.18.4', source: 'manifest' } : null
          }
        return {
          state: 'ok',
          version: ['3.4.1', '3.4.1', '3.3.8', '3.2.2'][hash % 4],
          ...
          extension:
            hash % 7 === 0
              ? null
              : { name: 'Hyper.Commerce', version: `2.19.${10 + (hash % 20)}`, source: 'manifest' }
        }
      }
      const jalaliToday = () => {
        try {
      ...
      async function demoDeploySteps(entry) {
        ...
        for (const co of entry.checkouts) {
          ...
          if (probe.state === 'offline') {
            step('connectivity', 'failed', `${co.hostname || co.ip} did not answer the ping`)
            results.push({
              checkoutId: co.id,
              name: co.name,
              host: co.hostname || co.ip,
              ok: false,
              error: 'Checkout unreachable',
              steps: [],
          ...
          await sleep(350)
          if ((co.id || 0) % 2 === 0) {
            step('backup', 'done', `${entry.fileName} → ${jalaliToday()}-${entry.fileName}`)
          } else {
            step('backup', 'skipped', 'No existing file — nothing to back up')
          }
          step('copy', 'running', `Copying ${entry.fileName}… (attempt 1/3)`)
          for (let percent = 0; percent <= 100; percent += 10) {
            buses.progress.emit({ runId: entry.runId, checkoutId: co.id, percent, attempt: 1 })
            await sleep(120)
          }
      ...
      /**
       * Demo import: mirrors the real pipeline step for step (same step keys,
       * same running → done statuses, byte progress on the copy) and honours
       * cancelAgentImport, so the dialog — including its Stop button — can be
       * exercised in the browser preview exactly as on Windows.
       */
      const demoAgentRuns = new Map()
      async function demoImportAgent({ checkout, runId }) {
        const copied = !importedAgents.has(checkout.id)
        const ownRun = !runId
        const id = runId || `demo-agent-${Date.now()}`
        if (ownRun && !demoAgentRuns.has(id)) demoAgentRuns.set(id, { cancelled: false })
        const run = demoAgentRuns.get(id) || { cancelled: false }
        const emit = (step: any, status: any, detail: any, progress?: any) =>
          buses.agentStep.emit({
        ...
        const cancelledError = () =>
          Object.assign(new Error('Import stopped by the operator'), {
            code: 'AGENT_IMPORT_CANCELLED',
            cancelled: true
          })
        const stopped = () => {
        ...
        try {
          stopped()
          emit('source', 'running', 'Hashing the bundled agent (demo)')
          await sleep(320)
          emit('source', 'done', 'Bundled agent SHA-256: 4f2c…a91d')
          stopped()
          emit('target', 'running', 'Checking C:\\Agent and its permissions')
          await sleep(280)
          emit('target', 'done', 'Agent directories verified and protected')
          emit('lock', 'done', 'Import lock acquired')
          emit(
            'compare',
            'running',
            copied ? 'Agent EXE is missing — copy required' : 'Reading installed agent SHA-256 over SMB'
          )
          await sleep(360)
          emit(
            'compare',
            'done',
            copied ? 'Agent EXE is missing — copy required' : 'SHA-256 matches — copy skipped'
          )
          stopped()
          if (copied) {
            const totalBytes = 2411520
            emit('copy', 'running', 'Copying staged agent: 0.0 / 2.4 MB (0%)')
            for (let percent = 0; percent <= 100; percent += 20) {
              stopped()
              emit(
                'copy',
                'running',
                `Copying staged agent: ${((totalBytes * percent) / 1000000).toFixed(1)} / 2.4 MB (${percent}%), 1840 KB/s`,
                {
                  bytes: Math.round((totalBytes * percent) / 100),
                  totalBytes,
                  elapsedMs: percent * 12,
                  bytesPerSecond: 1840000
                }
              )
              await sleep(130)
            }
            emit('copy', 'done', 'Staged agent copied and SHA-256 verified')
          }
          stopped()
          emit('service', 'running', 'Configuring the automatic Windows service')
          await sleep(340)
          emit('service', 'done', 'Automatic startup configured and the service started')
          emit('heartbeat', 'running', 'Waiting for a fresh agent heartbeat')
          await sleep(460)
          stopped()
          emit('heartbeat', 'done', 'A fresh agent heartbeat was verified')
          emit(
            'finish',
            'done',
            copied
              ? 'Copied, verified and the service is Running'
              : 'SHA-256 matched — the service is Running'
          )
          ...
          if (run.cancelled) {
            emit('rollback', 'done', 'Stopped by the operator — previous agent left in place')
            emit('cancelled', 'done', 'Import stopped by the operator')
            return {
              checkoutId: checkout.id,
              name: checkout.name,
              ok: false,
              cancelled: true,
              error: 'Import stopped by the operator',
              durationMs: Date.now() - startedAt
            }
          }
          emit('failed', 'done', error.message)
          return {
            checkoutId: checkout.id,
            name: checkout.name,
            ok: false,
          ...
          if (ownRun) demoAgentRuns.delete(id)
        }
      }
      /**
       * Demo install: mirrors the real Update Store Commerce pipeline step for
       * step (same step keys and statuses), including the per-checkout answer
       * (exit code, installer output, versions) the info popup shows.
       */
      const demoInstallRuns = new Map()
      const demoInstallState = new Map()
      async function demoInstallOne({ checkout, destinationPath, runId }: any) {
        const id = runId || `demo-install-${Date.now()}`
        if (!demoInstallRuns.has(id)) demoInstallRuns.set(id, { cancelled: false })
        const run = demoInstallRuns.get(id)
        const startedAt = Date.now()
        const steps = []
        const emit = (step, status, detail) => {
          steps.push({ step, status, detail })
          buses.installStep.emit({
            runId: id,
            checkoutId: checkout.id,
            name: checkout.name,
            step,
        ...
        const stopped = () => {
          if (run.cancelled) throw Object.assign(new Error('Stopped by the operator'), { cancelled: true })
        }
        ...
        const hash = ((checkout.id || 0) * 7919 + String(checkout.name || '').length * 31) % 997
        const base = demoVersion(checkout)
        try {
          stopped()
          emit('reachability', 'running', `Checking file sharing on ${checkout.ip || checkout.hostname}…`)
          await sleep(450)
          if (base.state === 'offline') {
            emit('reachability', 'failed', 'SMB (port 445) did not answer')
            return finish(false, { error: 'Checkout unreachable' })
          }
          emit('reachability', 'done', `SMB (port 445) answered in ${base.pingTime || 9} ms`)
          stopped()
          emit('agent', 'running', 'Checking the HyperFamily Agent service…')
          await sleep(420)
          if (!importedAgents.has(checkout.id)) {
            emit('agent', 'failed', 'Agent is not running')
            return finish(false, { error: 'Agent is not running — use Import Agent first' })
          }
          emit('agent', 'done', 'Agent 3.1.4 is running')
          const versionBefore = base.version || '9.52.24020.3'
          const wasRunning = hash % 10 < 6
          stopped()
          emit('running-check', 'running', 'Asking the agent whether Store Commerce is open…')
          await sleep(480)
          if (wasRunning) {
            emit('running-check', 'done', `Store Commerce is open — PID ${4200 + (hash % 700)}`)
            emit('close', 'running', 'Asking Store Commerce to close, force-stopping if it refuses…')
            await sleep(560)
            emit('close', 'done', 'Store Commerce was closed')
          } else {
            emit('running-check', 'done', 'Store Commerce is not running')
            emit('close', 'skipped', 'Store Commerce was already closed')
          }
          stopped()
          emit('verify-closed', 'running', 'Verifying that Store Commerce is closed…')
          await sleep(400)
          emit('verify-closed', 'done', 'No Store Commerce process is left')
          stopped()
          emit(
            'file-check',
            'running',
            `Looking for Hyper.StoreCommerce.Installer.exe in ${destinationPath}…`
          )
          await sleep(380)
          if (hash % 13 === 3) {
            emit(
              'file-check',
              'failed',
              `Hyper.StoreCommerce.Installer.exe was not found in ${destinationPath}`
            )
            emit('version', 'done', `Store Commerce v${versionBefore}`)
            return finish(false, {
              error: 'Hyper.StoreCommerce.Installer.exe not found — deploy it first',
          ...
          emit(
            'file-check',
            'done',
            `Hyper.StoreCommerce.Installer.exe (4812032 bytes) is in ${destinationPath}`
          )
          stopped()
          emit('install', 'running', 'Running Hyper.StoreCommerce.Installer.exe install with system rights…')
          await sleep(1400)
          const failedInstall = hash % 17 === 5
          if (failedInstall) {
            emit('install', 'failed', 'The installer exited with code 1603')
            emit('version', 'done', `Store Commerce v${versionBefore}`)
            return finish(false, {
              error: 'Installer exit code 1603',
              exitCode: 1603,
              output:
                'Setup encountered a fatal error 1603 while installing Store Commerce.\nRolling back the installation… done.',
              timedOut: false,
              version: versionBefore,
              versionBefore
            })
          }
          emit('install', 'done', 'The installer finished with exit code 0')
          const versionAfter = '9.60.24100.1'
          emit('version', 'done', `Store Commerce v${versionAfter} (was v${versionBefore})`)
          const result = finish(true, {
            exitCode: 0,
            output:
              'Preparing installation…\nInstalling Store Commerce 9.60.24100.1…\nApplying configuration…\nInstallation completed successfully.',
            version: versionAfter,
            versionBefore
          })
          demoInstallState.set(checkout.id, result)
          return result
        } catch (error) {
          if (error.cancelled) {
            emit('cancelled', 'skipped', 'Stopped by the operator')
            return finish(false, { cancelled: true, error: error.message })
          }
          emit('error', 'failed', error.message)
          return finish(false, { error: error.message })
        } finally {
          if (!runId) demoInstallRuns.delete(id)
        }
      }
      return {
        importAgent: demoImportAgent,
        importAgentAll: async ({ checkouts, runId }) => {
          const id = runId || `demo-agent-all-${Date.now()}`
          if (!demoAgentRuns.has(id)) demoAgentRuns.set(id, { cancelled: false })
          const results = []
          let cancelledByOperator = false
          try {
            for (const checkout of checkouts) {
              if (demoAgentRuns.get(id)?.cancelled) {
                results.push({
                  checkoutId: checkout.id,
                  name: checkout.name,
                  ok: false,
              ...
              results.push(await demoImportAgent({ checkout, runId: id }))
            }
            cancelledByOperator = Boolean(demoAgentRuns.get(id)?.cancelled)
          } finally {
            demoAgentRuns.delete(id)
          }
          return {
            runId: id,
            ...
            cancelled: results.filter((row) => row.cancelled && !row.skipped).length,
        ...
        /** Stop button of the Import Agent dialog (demo: flags the run). */
        cancelAgentImport: ({ runId }: any = {}) => {
          const run = demoAgentRuns.get(String(runId || ''))
          if (!run) return { cancelled: false, active: false }
          run.cancelled = true
          return { cancelled: true, active: true, runId: String(runId) }
        },
        onAgentStep: (callback) => buses.agentStep.subscribe(callback),
        version: async ({ checkout }) => {
          await sleep(600 + (((checkout?.id || 0) * 37) % 700))
          const result = { checkoutId: checkout?.id, name: checkout?.name, ...demoVersion(checkout || {}) }
          if (checkout?.id != null) demoVersionCache[checkout.id] = result
          return result
        },
        versionCache: async () => ({ ...demoVersionCache }),
        ...
        deploy: async ({ checkout, source, destinationPath }) => {
          ...
          const fileName = String(source || 'demo-update.exe')
            .split(/[\\/]/)
            .pop()
        ...
        testAccess: async ({ host }) => {
          await sleep(700)
          return { ok: true, host, user: 'okcs\\administrator', durationMs: 690 }
        },
        installed: async ({ checkout }) => {
          await sleep(800)
          if (!importedAgents.has(checkout.id))
            throw new Error('Agent is not running — Import Agent first (demo)')
          const programs = [
            ...
            {
              name: 'Store Commerce Hardware Station',
              version: '9.52.0.0',
              publisher: 'Microsoft Corporation'
            }
          ]
        ...
        install: async ({ checkout, destinationPath }) => demoInstallOne({ checkout, destinationPath }),
        installAll: async ({ checkouts = [], destinationPath, runId }) => {
          const id = runId || `demo-install-all-${Date.now()}`
          if (!demoInstallRuns.has(id)) demoInstallRuns.set(id, { cancelled: false })
          const results = []
          try {
            for (const checkout of checkouts) {
              if (demoInstallRuns.get(id)?.cancelled) {
                results.push({
                  checkoutId: checkout.id,
                  name: checkout.name,
                  ok: false,
              ...
              results.push(await demoInstallOne({ checkout, destinationPath, runId: id }))
            }
          } finally {
            demoInstallRuns.delete(id)
          }
          const summary = {
            runId: id,
            total: results.length,
            ...
            skipped: results.filter((row) => row.skipped).length,
            cancelledByOperator: Boolean(demoInstallRuns.get(id)?.cancelled),
            results,
            durationMs: results.reduce((sum, row) => sum + (row.durationMs || 0), 0)
          }
          buses.installFinished.emit(summary)
          return summary
        },
        cancelInstall: ({ runId }: any = {}) => {
          const run = demoInstallRuns.get(String(runId || ''))
          if (!run) return { cancelled: false }
          run.cancelled = true
          return { cancelled: true }
        },
        onInstallStep: (cb) => buses.installStep.subscribe(cb),
        onInstallFinished: (cb) => buses.installFinished.subscribe(cb),
