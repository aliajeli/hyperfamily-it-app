...
export class TerminalEmulator {
  ...
  eraseInLine(mode) {
    const line = this.lineAt(this.cursor.y)
    if (mode === 1)
      for (let index = 0; index <= this.cursor.x && index < this.cols; index += 1) line[index] = blankCell()
  ...
  applySgr(params) {
    ...
    for (let index = 0; index < values.length; index += 1) {
      ...
      if (code === 0) Object.assign(attr, DEFAULT_ATTR)
        ...
        if (values[index + 1] === 5) {
          attr[target] = PALETTE_16[values[index + 2] % 16] || null
          index += 2
        } else if (values[index + 1] === 2) {
          attr[target] = `rgb(${values[index + 2]} ${values[index + 3]} ${values[index + 4]})`
          index += 4
        }
      }
    }
  ...
  handleCsi(sequence) {
    ...
    switch (command) {
      case 'A':
        this.cursor.y = Math.max(0, this.cursor.y - Math.max(1, first))
        break
      case 'B':
        this.cursor.y = Math.min(this.rows - 1, this.cursor.y + Math.max(1, first))
        break
      case 'C':
        this.cursor.x = Math.min(this.cols - 1, this.cursor.x + Math.max(1, first))
        break
      case 'D':
        this.cursor.x = Math.max(0, this.cursor.x - Math.max(1, first))
        break
      case 'E':
        this.cursor.x = 0
        this.cursor.y = Math.min(this.rows - 1, this.cursor.y + Math.max(1, first))
      ...
      case 'F':
        ...
        break
      case 'G':
        this.cursor.x = Math.min(this.cols - 1, Math.max(0, (first || 1) - 1))
        break
      case 'H':
      case 'f':
      ...
      case 'X': {
        const line = this.lineAt(this.cursor.y)
        for (let index = 0; index < Math.max(1, first) && this.cursor.x + index < this.cols; index += 1)
          line[this.cursor.x + index] = blankCell()
        break
      }
      case 'm':
    ...
    for (let row = 0; row < this.rows; row += 1) {
      ...
      for (let column = 0; column < this.cols; column += 1) {
        ...
        if (current && current.attr === cell.attr) current.text += cell.char
          ...
          current = { attr: cell.attr, text: cell.char }
          runs.push(current)
        }
      }
      rows.push(runs)
