...
function TerminalWorkspaceInner() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [targets, snippetRows, settings] = await Promise.all([
        terminalApi.targets(),
        api.snippets.list(),
      ...
      setActiveBranchId((current) =>
        current && targets.some((branch) => branch.id === current) ? current : targets[0]?.id || null
      )
    } catch (error) {
      toast.error(error.message)
    } finally {
  ...
  useEffect(
    () => () => {
      if (sessionRef.current) terminalApi?.close(sessionRef.current.sessionId).catch(() => {})
    },
    [terminalApi]
  )
  ...
  const runSnippet = async (snippet) => {
    ...
    const lines = String(snippet.command || '')
      .replace(/\r\n?/g, '\n')
      .split('\n')
        ...
        >
                  ...
                  return (
                    ...
                    >
                      {busyDeviceId === device.id ? (
                        ...
                        <Cable
                          size={14}
                          className={cn('text-[rgb(var(--muted))]', isActive && 'text-[rgb(var(--primary))]')}
                        />
                      )}
                      <span className="min-w-0">
                        <span className="block truncate text-xs font-bold leading-tight">{device.name}</span>
                        <span className="block truncate font-mono text-xs text-[rgb(var(--muted))]">
                          {device.ip}
                        </span>
                      </span>
                      <span
            ...
            {session ? (
              <div className="flex min-h-0 flex-1 flex-col gap-2">
                <div className="flex flex-wrap items-center gap-2 rounded-xl border bg-[rgb(var(--surface))] px-3 py-1.5 text-2xs">
                  <Circle
                    ...
                    className={status.state === 'connecting' ? 'animate-pulse' : undefined}
                  />
                  ...
                  <span className="rounded-md bg-[rgb(var(--border)/.7)] px-1.5 py-0.5 text-xs font-extrabold uppercase">
                    {session.transport}
                  </span>
                  ...
                  <span className="ml-auto font-mono text-xs text-[rgb(var(--muted))]">
                    {grid.cols}×{grid.rows}
                  </span>
                  <Button
                    variant="ghost"
                    size="sm"
                    onClick={() => {
                      navigator.clipboard?.writeText(`${session.host}:${session.port}`)
                  ...
                  {live ? (
                    ...
                    <Button variant="ghost" size="sm" onClick={() => setSession(null)} title="Close pane">
                      <X size={13} />
                    </Button>
                  )}
                </div>
              ...
              <div className="grid min-h-0 flex-1 place-items-center rounded-2xl border border-dashed bg-[rgb(var(--surface)/.5)]">
                <EmptyState
                  ...
                  description={
                    branches.length
                      ? 'Pick a branch above, then choose a switch to open its console.'
                      : 'Add a switch to a branch to start using the terminal.'
                  }
                />
              </div>
            )}
          ...
          {fullscreen && (
            ...
            >
              ...
              <span className="text-xs font-bold uppercase tracking-widest [writing-mode:vertical-rl]">
                Snippets
              </span>
            </button>
          )}
