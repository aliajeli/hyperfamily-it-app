...
function groupCheckouts(branches: any, devices: any) {
  ...
  return [...groups.values()]
    .map((group: any) => ({
      ...group,
      checkouts: group.checkouts.sort((a: any, b: any) => (a.checkout_number ?? 999) - (b.checkout_number ?? 999) || a.name.localeCompare(b.name))
    }))
...
export default function StoreUpdatePage() {
  ...
  const [agentRun, setAgentRun] = useState<any>({ open: false, running: false, cancelling: false, cancelled: false, runId: null, targets: [], results: [], steps: {}, summary: null })
  const agentBusyRef = useRef(false)
  const agentRunIdRef = useRef<any>(null)
  const [dialog, setDialog] = useState<any>({ open: false, run: null })
  const [selected, setSelected] = useState(() => new Set())
  const [installRun, setInstallRun] = useState<any>({ open: false, running: false, cancelling: false, runId: null, targets: [], results: [], steps: {}, summary: null })
  ...
  useEffect(() => {
    ...
    const unsubs = [
      api.storeUpdate.onAgentStep((entry: any) => {
        setAgentRun((previous: any) => {
          ...
          const next = entry.progress && entries.at(-1)?.progress && entries.at(-1).step === entry.step ? [...entries.slice(0, -1), entry] : [...entries, entry]
      ...
      api.storeUpdate.onStep((entry: any) => {
        setSteps((previous: any) => ({ ...previous, [entry.checkoutId]: [...(previous[entry.checkoutId] || []), entry] }))
      }),
    ...
    return () => { alive = false; unsubs.forEach((unsub: any) => unsub?.()) }
  }, [])
  ...
  useEffect(() => {
    ...
    getApi().storeUpdate.versionCache?.()
      .then((cache: any) => { if (alive && cache) setVersions((previous: any) => ({ ...cache, ...previous })) })
      .catch(() => {})
    return () => { alive = false }
  }, [])

  const adoptProgramName = useCallback(async (name: any) => {
    try {
  ...
  const recheck = useCallback(async (checkout: any) => {
    if (!settings) return
    ...
    try {
      ...
      setVersions((previous: any) => ({ ...previous, [checkout.id]: { state: 'error', error: error.message } }))
      toast.error(error.message)
    }
  }, [settings])
  ...
  const importAgents = async (targets: any, all = false) => {
    ...
    const accepted = await confirm({
      ...
      description: 'The bundled EXE will be compared using SHA-256 and copied to C:\\Agent only if missing or different.',
    ...
    try {
      ...
      const summary = all ? await api.importAgentAll({ checkouts: targets, runId }) : await api.importAgent({ checkout: targets[0], runId }).then((result: any) => ({
      ...
      setAgentRun((previous: any) => ({ ...previous, running: false, cancelling: false, cancelled: Boolean(summary.cancelledByOperator), results: summary.results, summary: { ...summary, durationMs: summary.durationMs ?? summary.results?.reduce((sum: any, row: any) => sum + (row.durationMs || 0), 0) } }))
      ...
      setAgentRun((previous: any) => ({ ...previous, running: false, cancelling: false, results: targets.map((c: any) => ({ checkoutId: c.id, ok: false, error: error.message })), summary: { total: targets.length, ok: 0, failed: targets.length, cancelled: 0, skipped: 0, cancelledByOperator: false, results: [], durationMs: 0 } }))
      toast.error(error.message)
  ...
  const stopAgentImport = async () => {
    ...
    const accepted = await confirm({ title: 'Stop the agent import?', description: 'Current checkout will be rolled back and remaining skipped.', confirmLabel: 'Stop import', destructive: true })
    if (!accepted) return
  ...
  const startInstall = async (targets: any, { all = false }: any = {}) => {
    ...
    const accepted = await confirm({
      title: all ? `Update Store Commerce on all ${targets.length}?` : targets.length === 1 ? `Update Store Commerce on ${targets[0].name}?` : `Update ${targets.length} selected?`,
    ...
    try {
      ...
      setInstallRun((previous: any) => ({ ...previous, running: false, cancelling: false, results: summary.results, summary: { ...summary, durationMs: summary.durationMs ?? summary.results?.reduce((sum: any, row: any) => sum + (row.durationMs || 0), 0) } }))
      ...
      setInstallRun((previous: any) => ({ ...previous, running: false, cancelling: false, results: targets.map((c: any) => ({ checkoutId: c.id, ok: false, error: error.message, steps: [] })), summary: { total: targets.length, ok: 0, failed: targets.length, skipped: 0, cancelledByOperator: false, results: [], durationMs: 0 } }))
      toast.error(error.message)
  ...
  const stopInstall = async () => {
    ...
    const accepted = await confirm({ title: 'Stop the Store Commerce update?', description: 'Current checkout finishes step, remaining skipped.', confirmLabel: 'Stop update', destructive: true })
    if (!accepted) return
  ...
  const beginRun = (mode: any, checkouts: any) => {
    setSteps({})
    setProgress({})
    setDialog({ open: true, run: { mode, fileName: file.name, destinationPath: settings.store_update_path, checkouts, activeId: checkouts[0]?.id, summary: null } })
    setDeploying(true)
  }

  const finishRun = (summary: any) => {
    setDeploying(false)
    setDialog((previous: any) => previous.run ? { open: true, run: { ...previous.run, summary } } : previous)
  ...
  const deployAll = async () => {
    if (anyDeployRunning) return
    if (!file) { toast.error('Choose the update file first'); return }
    if (!allCheckouts.length) { toast.error('No checkout registered'); return }
    ...
    const accepted = await confirm({
      ...
      confirmLabel: 'Deploy to all', destructive: false
    })
    if (!accepted) return
  ...
  useEffect(() => {
    ...
    setDialog((previous: any) => (previous.run && previous.run.activeId !== activeId ? { ...previous, run: { ...previous.run, activeId } } : previous))
  }, [steps, deploying])
    ...
    <AppShell>
      <div className="mx-auto max-w-[1600px] space-y-3 px-2 md:px-0">
        ...
        <UpdateToolbar
          ...
          onUpdateSelected={() => startInstall(allCheckouts.filter((c: any) => selected.has(c.id)), {})}
        ...
        {loading ? <CheckoutsSkeleton /> : allCheckouts.length === 0 ? (
          <EmptyState icon={<ShoppingCart size={26} />} title="No checkout registered" description="Add checkout devices under Branches & devices first." />
        ) : (
          groups.map((group: any) => (
            <BranchCheckoutsCard
              ...
              agentBusyIds={agentRun.running ? new Set(agentRun.targets.map((target: any) => target.id)) : EMPTY_ID_SET}
              anyDeployRunning={anyDeployRunning}
      ...
      {installRun.running && !installRun.open && (
        <RunningPill kind="install" label={`Store Commerce update running — ${installRun.targets.length} checkout(s)`} onClick={() => setInstallRun((p: any) => ({ ...p, open: true }))} />
      )}
      {agentRun.running && !agentRun.open && (
        <RunningPill kind="agent" label={`Agent import running — ${agentRun.targets.length} checkout(s)`} onClick={() => setAgentRun((p: any) => ({ ...p, open: true }))} />
      )}

      <ServerFileBrowser open={browserOpen} onOpenChange={setBrowserOpen} onSelect={(path: any) => { setFile({ path, name: path.split(/[\\/]/).pop() }); toast.success(`Selected ${path.split(/[\\/]/).pop()}`) }} />

      <AgentImportDialog run={agentRun} onCancel={stopAgentImport} onClose={() => setAgentRun((p: any) => ({ ...p, open: false }))} />
      <InstalledProgramsDialog open={inspect.open} onOpenChange={(open: any) => setInspect((p: any) => ({ ...p, open }))} checkout={inspect.checkout} onAdopt={adoptProgramName} />
      <DeployDialog open={dialog.open} onOpenChange={(open: any) => setDialog((p: any) => ({ ...p, open }))} run={dialog.run ? { ...dialog.run, steps, progress } : null} running={deploying} onClose={() => setDialog((p: any) => ({ ...p, open: false }))} />
      <StoreInstallDialog open={installRun.open} onOpenChange={(open: any) => setInstallRun((p: any) => ({ ...p, open }))} run={installRun.runId ? { mode: installRun.targets.length > 1 ? 'all' : 'single', checkouts: installRun.targets, steps: installRun.steps, activeId: installRun.activeId, summary: installRun.summary, cancelling: installRun.cancelling } : null} running={installRun.running} onCancel={stopInstall} onClose={() => setInstallRun((p: any) => ({ ...p, open: false }))} />
      <StoreInstallDialog open={Boolean(installView.open && installView.checkout && installResults[installView.checkout.id])} onOpenChange={(open: any) => setInstallView((p: any) => ({ ...p, open }))} run={installView.checkout && installResults[installView.checkout.id] ? { mode: 'single', checkouts: [installView.checkout], steps: { [installView.checkout.id]: installResults[installView.checkout.id].steps || [] }, summary: { total: 1, ok: installResults[installView.checkout.id].ok ? 1 : 0, failed: installResults[installView.checkout.id].ok ? 0 : 1, skipped: 0, results: [installResults[installView.checkout.id]], durationMs: installResults[installView.checkout.id].durationMs || 0 }, cancelling: false } : null} running={false} onClose={() => setInstallView((p: any) => ({ ...p, open: false }))} />
    </AppShell>
  )
}
