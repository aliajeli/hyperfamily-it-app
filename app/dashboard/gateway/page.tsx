    ...
    return (
      <div className="mx-auto max-w-[1100px]">
        <EmptyState
          ...
          action={
            <Button variant="secondary" onClick={() => navigation.push('/dashboard')}>
  ...
  const metricCards = [
    {
      ...
      value: metrics.current == null ? 'No reply' : `${metrics.current} ms`,
      icon: Activity,
      tone:
        metrics.current != null && metrics.current <= 300
          ? 'status-online-text bg-nord-14/16'
          : 'status-warning-text bg-nord-13/18'
    },
    {
      title: 'Average latency',
      value: metrics.average == null ? '—' : `${metrics.average} ms`,
      icon: Gauge,
      tone: 'text-nord-10 bg-nord-8/16'
    },
    {
      title: 'Healthy responses',
      value: metrics.availability == null ? '—' : `${metrics.availability}%`,
      icon: ShieldCheck,
      tone: 'status-online-text bg-nord-14/16'
    },
    ...
    <div className="mx-auto max-w-[1500px] space-y-5">
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div className="flex min-w-0 items-center gap-3">
          ...
          <div className="min-w-0">
            <p className="text-xs font-extrabold uppercase tracking-[0.16em] text-[rgb(var(--muted))]">
              {branch.name} · {branch.code}
            </p>
            <h1 className="truncate text-2xl font-extrabold tracking-tight">Router connection details</h1>
            <p className="mt-0.5 font-mono text-2xs text-[rgb(var(--muted))]">
              {gateway.name} · {gateway.ip_address || gateway.ip}
            </p>
          </div>
        </div>
        ...
        {metricCards.map((metric) => {
          ...
          return (
            <Card key={metric.title} className="flex items-center gap-3 p-4">
              <div className={`grid h-10 w-10 shrink-0 place-items-center rounded-xl ${metric.tone}`}>
                <Icon size={18} />
              </div>
              <div className="min-w-0">
                <p className="truncate text-xs font-extrabold uppercase tracking-[0.12em] text-[rgb(var(--muted))]">
                  {metric.title}
                </p>
      ...
      <Card className="overflow-hidden">
        <div className="flex flex-col justify-between gap-2 border-b px-5 py-4 sm:flex-row sm:items-center">
          <div>
            ...
            <p className="mt-0.5 text-xs text-[rgb(var(--muted))]">
              Every response through 300 ms is healthy and shown in the green range. Responses above 300 ms
              are warnings.
            </p>
          </div>
          <div className="flex flex-wrap gap-2 text-xs font-bold tabular-nums text-[rgb(var(--muted))]">
            <span className="rounded-lg bg-[rgb(var(--canvas))] px-2.5 py-1.5">
              Min {metrics.minimum == null ? '—' : `${metrics.minimum} ms`}
            </span>
            <span className="rounded-lg bg-[rgb(var(--canvas))] px-2.5 py-1.5">
              Max {metrics.maximum == null ? '—' : `${metrics.maximum} ms`}
            </span>
            <span className="status-online-text rounded-lg bg-nord-14/14 px-2.5 py-1.5">
              Healthy ≤ 300 ms
            </span>
          </div>
        </div>
        <div className="p-4 sm:p-6">
          {gateway.history?.length ? (
            <PingChart history={gateway.history} detailed />
          ) : (
            <div className="grid h-80 place-items-center rounded-xl border border-dashed text-sm text-[rgb(var(--muted))]">
              Waiting for Router ping samples…
            </div>
          )}
        </div>
      </Card>
      ...
      <Card className="overflow-hidden">
        <div className="border-b px-5 py-4">
          ...
          <p className="mt-0.5 text-xs text-[rgb(var(--muted))]">
            Newest monitoring results are listed first.
          </p>
        </div>
        {gateway.history?.length ? (
              ...
              <tbody className="divide-y">
                {[...gateway.history]
                  ...
                  .map((probe, index) => {
                    ...
                    return (
                      <tr
                        key={`${probe.checked_at || probe.timestamp || 'probe'}-${index}`}
                        className="hover:bg-[rgb(var(--border)/.18)]"
                      >
                        <td className="px-5 py-3 font-medium">
                          {formatTimestamp(probe.checked_at || probe.timestamp)}
                        </td>
                        <td className="px-5 py-3 font-mono font-bold tabular-nums">
                          {replied ? `${responseTime} ms` : 'No reply'}
                        </td>
                        <td className="px-5 py-3">
                          <span
                            className={`inline-flex items-center gap-1.5 rounded-full border px-2 py-1 text-xs font-extrabold uppercase tracking-wider ${healthy ? 'status-online-text border-nord-14/40 bg-nord-14/15' : replied ? 'status-warning-text border-nord-13/50 bg-nord-13/18' : 'border-nord-11/40 bg-nord-11/12 text-nord-11'}`}
                          >
                            {healthy ? <ShieldCheck size={10} /> : <WifiOff size={10} />}
                            {healthy ? 'Healthy' : replied ? 'Warning' : 'Offline'}
                          </span>
                        </td>
                        <td className="px-5 py-3 capitalize text-[rgb(var(--muted))]">
          ...
          <div className="p-8 text-center text-sm text-[rgb(var(--muted))]">
            No probe history is available yet.
          </div>
        )}
      </Card>
    </div>
