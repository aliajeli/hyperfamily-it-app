                ...
                return (
                  ...
                  >
                    ...
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-xs font-bold text-[rgb(var(--text))]">{program.name}</span>
                      {program.publisher && <span className="block truncate text-xs text-[rgb(var(--muted))]">{program.publisher}</span>}
                    </span>
                    <span className="shrink-0 font-mono text-2xs font-bold text-[rgb(var(--muted))]">{program.version || '—'}</span>
                  </button>
                )
              })}
              {rows.length === 0 && <p className="py-6 text-center text-2xs text-[rgb(var(--muted))]">Nothing matches that filter.</p>}
            </div>
          </>
        )}
