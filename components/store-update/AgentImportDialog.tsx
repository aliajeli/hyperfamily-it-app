...
function pipelineSteps(steps, settled) {
  ...
  const merged = PIPELINE.map(
    (step) => byStep.get(step) || { step, status: 'pending', detail: settled ? 'Not reached' : 'Waiting' }
  )
...
function StepRow({ entry }: any) {
  const percent =
    entry.progress && entry.progress.totalBytes > 0 && entry.status === 'running'
  ...
  return (
    <div className="min-w-0">
      <div className="flex items-center gap-2">
        ...
        <span
          className="min-w-0 flex-1 truncate font-mono text-xs text-[rgb(var(--muted))]"
          title={entry.detail}
        >
          {entry.detail}
        </span>
        {percent != null && (
          <span className="shrink-0 font-mono text-xs font-bold text-[rgb(var(--primary))]">{percent}%</span>
        )}
      </div>
      {percent != null && (
 ...
 */
export default function AgentImportDialog({ run, onCancel, onClose }: any) {
  if (!run) return null
  ...
  const failed =
    (summary?.failed ?? run.results?.filter((entry) => !entry.ok && !entry.cancelled).length) || 0
  const succeeded = summary?.ok ?? run.results?.filter((entry) => entry.ok).length ?? 0
  const stoppedCount =
    summary?.cancelled ?? run.results?.filter((entry) => entry.cancelled && !entry.skipped).length ?? 0
      ...
      <div className="space-y-3">
        ...
        {done && (
          <div
            className={cn(
              ...
              cancelled && failed === 0
                ? 'border-nord-13/45 bg-nord-13/12 status-warning-text'
                : failed > 0
          ...
          >
            <span className="flex items-center gap-1.5">
              <CheckCircle2 size={14} className="text-nord-14" />
              {succeeded} succeeded
            </span>
            {failed > 0 && (
              <span className="flex items-center gap-1.5">
            ...
            {summary?.durationMs > 0 && (
              <span className="ml-auto font-semibold text-[rgb(var(--muted))]">
        ...
        {!done && (
          <p className="text-2xs leading-relaxed text-[rgb(var(--muted))]">
            Identical EXEs are not copied again. Slow branch transfers may continue for up to 30 minutes per
            file operation while making progress; 2 minutes without I/O progress stops the operation. The
            service starts before Login and must produce a fresh heartbeat before the import succeeds.
          </p>
        )}
              ...
              <motion.section
                ...
                className={cn(
                  ...
                  (state === 'pending' || state === 'skipped') &&
                    'border-[rgb(var(--border)/.6)] bg-[rgb(var(--surface)/.45)] opacity-80'
                )}
              >
                <header className="flex items-center gap-2">
                  {state === 'running' ? (
                    <Loader2 size={15} className="animate-spin text-[rgb(var(--primary))]" />
                  ) : state === 'done' ? (
                    <CheckCircle2 size={15} className="text-nord-14" />
                  ) : state === 'failed' ? (
                    <XCircle size={15} className="text-nord-11" />
                  ) : state === 'cancelled' ? (
                    <OctagonX size={15} className="status-warning-text" />
                  ) : state === 'skipped' ? (
                    <Ban size={15} className="text-[rgb(var(--muted))]" />
                  ) : (
                    <ShieldCheck size={15} className="text-[rgb(var(--muted))]" />
                  )}
                  <span className="min-w-0 flex-1 truncate text-sm font-bold text-[rgb(var(--text))]">
                    {checkout.name}
                  </span>
                  {/* The address the import really connects to. */}
                  <span
                    className="truncate font-mono text-xs text-[rgb(var(--muted))]"
                    title={[checkout.hostname, checkout.ip].filter(Boolean).join(' · ')}
                  >
                    {checkout.ip || checkout.hostname}
                  </span>
                  {result?.ok && (
                    <span className="shrink-0 rounded-full bg-nord-14/20 px-2 py-0.5 text-xs font-bold text-[#5c7a46] dark:text-nord-14">
                      {result.copied ? 'Copied and verified' : 'SHA-256 matched'}
                  ...
                  {result && !result.ok && (
                    <span
                      className={cn(
                        'shrink-0 max-w-[46%] truncate rounded-full px-2 py-0.5 text-xs font-bold',
                        result.cancelled ? 'bg-nord-13/20 status-warning-text' : 'bg-nord-11/15 text-nord-11'
                      )}
                      title={result.error}
                    >
                ...
                {steps.length > 0 && (
                  <div className="mt-2 space-y-1 border-t border-[rgb(var(--border)/.45)] pt-2">
                    {pipelineSteps(steps, !isActive).map((entry) => (
          ...
          <p className="min-w-0 flex-1 text-2xs text-[rgb(var(--muted))]">
            {running ? (
              <span className="inline-flex items-center gap-1.5">
                <CloudUpload size={12} /> Checkouts are imported strictly one after another — keep the app
                open.
              </span>
            ) : cancelled ? (
