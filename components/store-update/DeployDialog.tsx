...
function StepRow({ entry, progress }: any) {
  ...
  return (
    <div className="min-w-0">
      <div className="flex items-center gap-2">
        ...
        <span
          className={cn(
            'w-[132px] shrink-0 text-2xs font-bold',
            entry.status === 'failed'
              ? 'text-nord-11'
 ...
 */
export default function DeployDialog({ open, onOpenChange, run, running, onClose }: any) {
  if (!run) return null
        ...
        {done && (
          ...
          >
            ...
            <span className="ml-auto font-semibold text-[rgb(var(--muted))]">
              {formatBytes(run.summary.results?.reduce((sum, r) => sum + (r.bytes || 0), 0))} deployed ·{' '}
                ...
                <header className="flex items-center gap-2">
                  ...
                  {result?.ok && (
                    <span className="shrink-0 rounded-full bg-nord-14/20 px-2 py-0.5 text-xs font-bold text-[#5c7a46]">
                      {formatBytes(result.bytes)} · {formatDuration(result.durationMs)}
                    </span>
                  )}
                </header>
                {steps.length > 0 && (
                  <div className="mt-2 space-y-1 border-t border-[rgb(var(--border)/.45)] pt-2">
                    {collapseSteps(steps, Boolean(result)).map((entry) => (
                      <StepRow
                        key={`${checkout.id}-${entry.step}`}
                        entry={entry}
                        progress={run.progress[checkout.id]}
                      />
                    ))}
                  </div>
                )}
          ...
          <p className="text-2xs text-[rgb(var(--muted))]">
            {running ? (
              <span className="inline-flex items-center gap-1.5">
                <CloudUpload size={12} /> Machines are updated strictly one after another — keep the app open.
              </span>
            ) : (
