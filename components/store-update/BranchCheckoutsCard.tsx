...
function VersionChip({ version }: any) {
  ...
  if (state === 'unknown') {
    return (
      <span className="inline-flex items-center rounded-full bg-[rgb(var(--border)/.5)] px-2 py-1 text-2xs font-bold text-[rgb(var(--muted))]">
        Not checked
      </span>
    )
  }
  if (state === 'checking') {
    return (
      <span className="inline-flex items-center gap-1 rounded-full bg-[rgb(var(--primary)/.1)] px-2 py-1 text-2xs font-bold text-[rgb(var(--primary))]">
  ...
  if (state === 'ok') {
    return (
      <span
        className="inline-flex items-center gap-1 rounded-full bg-nord-14/15 px-2.5 py-1 font-mono text-xs font-bold text-nord-14"
  ...
  return (
    <span
      className={cn('inline-flex items-center rounded-full px-2.5 py-1 text-2xs font-bold', look.className)}
...
function ExtensionChip({ version }: any) {
  ...
  if (!version.extension) {
    return (
      <span className="inline-flex items-center rounded-full bg-[rgb(var(--border)/.6)] px-2 py-1 text-2xs font-bold text-[rgb(var(--muted))]">
        No ext
      </span>
    )
  }
  return (
    <span
      className="inline-flex items-center gap-1 rounded-full bg-[rgb(var(--primary)/.12)] px-2 py-1 text-2xs font-bold text-[rgb(var(--primary))]"
...
export default function BranchCheckoutsCard({
  ...
  const offlineCount = states.filter(
    (state: any) => state === 'offline' || state === 'error' || state === 'agent-not-running'
  ).length
    ...
    <section className="overflow-hidden rounded-2xl border border-[rgb(var(--border)/.7)] bg-[rgb(var(--surface)/.6)] shadow-sm">
      <header className="flex flex-wrap items-center gap-2 border-b border-[rgb(var(--border)/.55)] bg-[rgb(var(--surface)/.8)] px-3 py-2.5">
        <button
          ...
          className="grid h-8 w-8 place-items-center rounded-lg text-[rgb(var(--muted))] transition hover:bg-[rgb(var(--border)/.5)]"
        >
          <ChevronDown size={16} className={cn('transition-transform', collapsed && '-rotate-90')} />
        </button>
        <span className="grid h-8 w-8 shrink-0 place-items-center rounded-lg bg-[rgb(var(--primary)/.12)] text-[rgb(var(--primary))]">
          <Building2 size={14} />
        </span>
        ...
        <div className="flex flex-wrap items-center gap-1 text-2xs font-bold md:ml-2">
          {checkingCount > 0 && (
            <span className="rounded-full bg-[rgb(var(--primary)/.12)] px-2 py-1 text-[rgb(var(--primary))]">
              {checkingCount} checking
            </span>
          )}
          {okCount > 0 && (
            <span className="rounded-full bg-nord-14/15 px-2 py-1 text-nord-14">{okCount} ok</span>
          )}
          {offlineCount > 0 && (
            <span className="rounded-full bg-nord-11/15 px-2 py-1 text-nord-11">{offlineCount} offline</span>
          )}
          {unknownCount > 0 && (
            <span className="rounded-full bg-[rgb(var(--border)/.6)] px-2 py-1 text-[rgb(var(--muted))]">
              {unknownCount} new
            </span>
          )}
        </div>
        <div className="ml-auto flex w-full items-center gap-1.5 md:ml-auto md:w-auto">
          ...
          <button
            ...
            className="flex flex-1 items-center justify-center gap-1 rounded-lg border bg-[rgb(var(--surface))] px-2.5 py-2 text-xs font-bold shadow-sm disabled:opacity-40 md:flex-none md:px-2 md:py-1 md:text-2xs"
          >
          ...
          <button
            ...
            className="flex flex-1 items-center justify-center gap-1 rounded-lg bg-[rgb(var(--primary))] px-2.5 py-2 text-xs font-bold text-white shadow-sm disabled:opacity-40 md:flex-none md:px-2 md:py-1 md:text-2xs"
          >
              ...
              return (
                <div key={checkout.id} className="rounded-xl border bg-[rgb(var(--surface))] p-3 shadow-sm">
                  <div className="flex items-start gap-2.5">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mt-0.5 flex flex-wrap items-center gap-1 font-mono text-2xs text-[rgb(var(--muted))]">
                        <Monitor size={10} /> {checkout.ip}{' '}
                        {checkout.hostname ? `• ${checkout.hostname}` : ''}
                      </p>
                      ...
                      {(version.detail || version.error) && (
                        <p className="mt-1.5 text-2xs leading-snug text-[rgb(var(--muted))]">
                          {version.detail || version.error}
                        </p>
                      )}
                    </div>
                  ...
                  <div className="mt-3 grid grid-cols-5 gap-1.5">
                    ...
                    >
                      <RefreshCw size={16} className={version?.state === 'checking' ? 'animate-spin' : ''} />
                    </RowAction>
                    <RowAction
                    ...
                    <RowAction
                      ...
                      label="Deploy"
                      title={hasFile ? 'Deploy file' : 'Select file first'}
                    >
                      <CloudUpload size={16} />
                    </RowAction>
                    <RowAction
          ...
          <ul className="hidden divide-y divide-[rgb(var(--border)/.4)] md:block">
            {checkouts.map((checkout: any) => {
              ...
              return (
                <li
                  key={checkout.id}
                  className="flex flex-wrap items-center gap-2 px-3 py-2 transition-colors hover:bg-[rgb(var(--surface)/.7)]"
                >
                  <input
                    type="checkbox"
                  ...
                  <span className="min-w-0 flex-1 truncate text-xs font-bold">
                    {checkout.name}
                    {checkout.checkout_number ? (
                      <span className="ml-1.5 font-mono text-2xs text-[rgb(var(--muted))]">
                        #{checkout.checkout_number}
                      </span>
                    ) : null}
                  ...
                  {installResult && (
                    <button
                      ...
                      onClick={() => onShowInstallResult(checkout)}
                      title={
                        installResult.ok ? `Updated to v${installResult.version || '?'}` : installResult.error
                      }
                      className={cn(
                  ...
                  <span className="ml-auto flex items-center gap-0.5">
                    <button
                      ...
                      onClick={() => onRecheck(checkout)}
                      disabled={busy}
                      className="grid h-7 w-7 place-items-center rounded-lg border border-transparent text-[rgb(var(--muted))] hover:bg-[rgb(var(--surface))] hover:text-[rgb(var(--text))] disabled:opacity-40"
                      title="Recheck"
                    >
                    ...
                    <button
                      ...
                      disabled={anyDeployRunning || agentBusy || !hasFile}
                      className="grid h-7 w-7 place-items-center rounded-lg border border-transparent text-[rgb(var(--muted))] hover:bg-[rgb(var(--surface))] hover:text-[rgb(var(--text))] disabled:opacity-40"
                      title={hasFile ? 'Deploy' : 'Select file first'}
                    >
                      <CloudUpload size={13} />
                    </button>
                    <button
