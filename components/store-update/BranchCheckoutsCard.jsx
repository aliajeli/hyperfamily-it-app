...
import { Building2, ChevronDown, ClipboardList, CloudUpload, Loader2, RefreshCw, ShieldCheck, Wrench } from 'lucide-react'
...
function VersionChip({ version }) {
  ...
  if (state === 'ok') {
    ...
    return (
      <span
        ...
        title={[provenance, version.pingTime != null ? `Responded in ${version.pingTime} ms` : null, version.icmp === false ? 'Ping is filtered on this host; reached over SMB' : null].filter(Boolean).join(' · ')}
      >
        v{version.version}
        {version.stale && <span className="font-sans font-bold text-[#8b6e1c]" title="A newer Store Commerce was detected elsewhere — this checkout is behind">~</span>}
      </span>
    )
  }
  const looks = {
  ...
  return <span className={cn('inline-flex items-center rounded-full px-2 py-0.5 text-2xs font-bold', look.className)} title={version?.error || version?.detail || undefined}>{look.label}</span>
}

/** Hyper.Commerce extension chip, shown once the checkout answered. */
function ExtensionChip({ version }) {
  const state = version?.state || 'checking'
  if (state !== 'ok' && state !== 'not-found') return null
  if (!version.extension) {
    return <span className="inline-flex items-center rounded-full bg-[rgb(var(--border)/.6)] px-2 py-0.5 text-2xs font-bold text-[rgb(var(--muted))]" title="The Hyper.Commerce extension was not found in its manifest, Programs and Features or its installation folder on this checkout">No extension</span>
  }
  return (
    <span
...
function RowAction({ onClick, disabled, label, title, children, danger }) {
  return (
    <button
      type="button"
      onClick={onClick}
  ...
  return (
    <section className="overflow-hidden rounded-2xl border border-[rgb(var(--border)/.7)] bg-[rgb(var(--surface)/.5)] shadow-sm">
      <header className="flex flex-wrap items-center gap-2 border-b border-[rgb(var(--border)/.55)] bg-[rgb(var(--surface)/.75)] px-3 py-2">
        <button type="button" onClick={() => setCollapsed((value) => !value)} aria-label={collapsed ? 'Expand branch' : 'Collapse branch'} className="grid h-7 w-7 place-items-center rounded-lg text-[rgb(var(--muted))] transition hover:bg-[rgb(var(--border)/.5)] hover:text-[rgb(var(--text))]">
          <ChevronDown size={15} className={cn('transition-transform', collapsed && '-rotate-90')} />
        </button>
        <span className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-[rgb(var(--primary)/.12)] text-[rgb(var(--primary))]"><Building2 size={14} /></span>
        <h2 className="truncate text-sm font-bold text-[rgb(var(--text))]">{branch.name}</h2>
        <span className="rounded-full bg-[rgb(var(--border)/.6)] px-2 py-0.5 font-mono text-2xs font-bold text-[rgb(var(--muted))]">{branch.code}</span>
        <span className="text-2xs font-semibold text-[rgb(var(--muted))]">{checkouts.length} checkout{checkouts.length !== 1 ? 's' : ''}</span>
        ...
        <div className="ml-auto flex items-center gap-1.5">
          <label className="flex cursor-pointer select-none items-center gap-1.5 rounded-lg px-1.5 py-1 text-2xs font-bold text-[rgb(var(--muted))] transition hover:text-[rgb(var(--text))]">
            <input
              ...
              checked={allSelected}
              onChange={() => onSelectMany(checkouts.map((checkout) => checkout.id), !allSelected)}
            />
            Select all
          </label>
          <button type="button" onClick={() => onRecheckBranch(checkouts)} disabled={anyDeployRunning || checkingCount > 0} className="flex items-center gap-1 rounded-lg border border-[rgb(var(--border)/.7)] px-2 py-1 text-2xs font-bold text-[rgb(var(--muted))] transition hover:text-[rgb(var(--text))] disabled:pointer-events-none disabled:opacity-40">
            <RefreshCw size={11} /> Recheck
          </button>
          <button type="button" onClick={() => onUpdateBranch(checkouts)} disabled={anyDeployRunning} className="flex items-center gap-1 rounded-lg bg-[rgb(var(--primary))] px-2 py-1 text-2xs font-bold text-white shadow-sm transition hover:opacity-90 disabled:pointer-events-none disabled:opacity-40">
      ...
      {!collapsed && (
        <ul className="divide-y divide-[rgb(var(--border)/.4)]">
          {checkouts.map((checkout) => {
            ...
            return (
              <li key={checkout.id} className="flex flex-wrap items-center gap-2 px-3 py-1.5 transition-colors hover:bg-[rgb(var(--surface)/.7)]">
                <input
                  type="checkbox"
                ...
                <span className="min-w-0 flex-1 truncate text-xs font-bold text-[rgb(var(--text))]">
                  {checkout.name}
                  {checkout.checkout_number ? <span className="ml-1.5 font-mono text-2xs font-semibold text-[rgb(var(--muted))]">#{checkout.checkout_number}</span> : null}
                  <span className="ml-1.5 font-mono text-2xs font-semibold text-[rgb(var(--muted))]">{checkout.ip}</span>
                </span>
                ...
                {installResult && (
                  <button
                    ...
                    title={installResult.ok ? `Updated to v${installResult.version || '?'}` : installResult.cancelled ? 'Update stopped' : (installResult.error || 'Update failed')}
                    className={cn('h-2.5 w-2.5 shrink-0 rounded-full ring-2 ring-[rgb(var(--surface))]', installResult.ok ? 'bg-nord-14' : 'bg-nord-11')}
                    aria-label="Show the last update result"
                  />
                )}
                <span className="ml-auto flex items-center gap-0.5">
                  <RowAction onClick={() => onRecheck(checkout)} disabled={busy} label="Recheck version" title="Recheck Store Commerce version">
                    <RefreshCw size={13} className={version?.state === 'checking' ? 'animate-spin' : ''} />
                  </RowAction>
                  <RowAction onClick={() => onImportAgent(checkout)} disabled={anyDeployRunning || agentBusy} label="Import agent" title="Install or update the HyperFamily agent on this checkout">
                    <ShieldCheck size={13} />
                  </RowAction>
                  <RowAction onClick={() => onDeploy(checkout)} disabled={anyDeployRunning || agentBusy || !hasFile} label="Deploy update file" title={hasFile ? 'Copy the selected update file to this checkout' : 'Select an update file first'}>
                    <CloudUpload size={13} />
                  </RowAction>
                  <RowAction onClick={() => onInspect(checkout)} disabled={busy} label="Inspect installed programs" title="List everything installed on this checkout">
                    <ClipboardList size={13} />
                  </RowAction>
                  <RowAction onClick={() => onUpdateStore(checkout)} disabled={anyDeployRunning || agentBusy} label="Update Store Commerce" title="Run the Store Commerce update pipeline on this checkout">
                    <Wrench size={13} />
                  </RowAction>
                </span>
              </li>
