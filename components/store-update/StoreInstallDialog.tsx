...
function StepRow({ entry }: any) {
  return (
    <div className="flex items-center gap-2">
      ...
      <span
        className={cn(
          'w-[150px] shrink-0 text-2xs font-bold',
          entry.status === 'failed'
            ? 'text-nord-11'
 ...
 */
function ResultInfo({ result }: any) {
    ...
    <div className="mt-2 border-t border-[rgb(var(--border)/.45)] pt-2">
      <button
        ...
        className={cn(
          ...
          ok
            ? 'bg-nord-14/12 text-[#4c6a3a] hover:bg-nord-14/20'
            : result?.cancelled
              ? 'bg-nord-13/15 text-[#8b6e1c] hover:bg-nord-13/25'
      ...
      {open && (
        <div className="mt-1.5 space-y-1.5 rounded-lg border border-[rgb(var(--border)/.5)] bg-[rgb(var(--surface)/.6)] p-2.5">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-2xs font-bold text-[rgb(var(--muted))]">
            <span>
              Exit code:{' '}
              <b className={cn('font-mono', ok ? 'text-nord-14' : 'text-nord-11')}>
                {result.exitCode ?? '—'}
              </b>
            </span>
            <span>
              Version:{' '}
              <b className="font-mono text-[rgb(var(--text))]">
                {result.versionBefore || '?'} → {result.version || '?'}
              </b>
            </span>
          ...
          {result.output ? (
            <div className="max-h-40 overflow-y-auto rounded-md bg-[#1d2129] p-2">
              <div className="mb-1 flex items-center gap-1 text-[10px] font-bold uppercase tracking-wide text-[#7b8496]">
                <Terminal size={10} /> Installer output
              </div>
              <pre className="whitespace-pre-wrap break-words font-mono text-[11px] leading-relaxed text-[#c8d0dc]">
                {result.output}
              </pre>
            </div>
          ) : (
 ...
 */
export default function StoreInstallDialog({ open, onOpenChange, run, running, onClose, onCancel }: any) {
  if (!run) return null
        ...
        {done && (
          ...
          >
            ...
            {run.summary.skipped > 0 && (
              <span className="flex items-center gap-1.5 text-nord-13">{run.summary.skipped} skipped</span>
            )}
            ...
            return (
              <motion.section
                ...
                className={cn(
                  ...
                  state === 'pending' &&
                    'border-[rgb(var(--border)/.6)] bg-[rgb(var(--surface)/.45)] opacity-75'
                )}
              >
                <header className="flex items-center gap-2">
                  ...
                  {result?.ok && (
                    <span className="shrink-0 rounded-full bg-nord-14/20 px-2 py-0.5 text-xs font-bold text-[#5c7a46]">
                      v{result.version || '?'} · exit {result.exitCode ?? 0}
                    </span>
                  )}
                  {result && !result.ok && (
                    <span
                      className="shrink-0 rounded-full bg-nord-11/15 px-2 py-0.5 text-xs font-bold text-nord-11"
                      title={result.error}
                    >
                      {result.error || 'Failed'}
                    </span>
