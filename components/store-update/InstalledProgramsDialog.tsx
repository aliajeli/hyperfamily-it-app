 ...
 */
export default function InstalledProgramsDialog({ open, onOpenChange, checkout, onAdopt }: any) {
  ...
  useEffect(() => {
    ...
    setQuery('')
    getApi()
      .storeUpdate.installed({ checkout })
      .then((data) => {
      ...
      .catch((error) => {
        if (alive)
          setState({
            loading: false,
            data: null,
            error: error.message.replace(/^Error invoking remote method '[^']+': (?:Error: )?/, '')
          })
      })
    return () => {
      alive = false
  ...
  const rows = useMemo(() => {
    ...
    return all.filter(
      (program) =>
        ...
        (program.publisher || '').toLowerCase().includes(needle)
    )
  }, [state.data, query])
  ...
  return (
    <Dialog
      ...
      description={checkout ? `Programs and Features as reported by ${checkout.name}` : ''}
      className="max-w-2xl"
    >
      <div className="space-y-2.5">
        ...
        {state.error && (
          <div className="flex items-start gap-2 rounded-xl border border-nord-11/35 bg-nord-11/10 p-2.5 text-2xs leading-relaxed text-nord-11">
        ...
        {state.data && (
          <>
            <p className="text-2xs text-[rgb(var(--muted))]">
              {state.data.source === 'agent'
                ? 'Source: running local Agent (Programs and Features).'
                : state.data.source === 'wmi'
                  ? 'Source: live registry via WMI (Programs and Features).'
                  : state.data.source === 'registry-backup'
                    ? 'Source: registry backup — versions may be outdated.'
                    : 'Source: live registry (Programs and Features).'}
            </p>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="shrink-0 rounded-full bg-[rgb(var(--border)/.6)] px-2.5 py-1 text-xs font-bold text-[rgb(var(--muted))]">
                {rows.length} of {state.data.total}
              </span>
            </div>

            <p className="rounded-xl border border-[rgb(var(--border)/.55)] bg-[rgb(var(--surface)/.45)] px-2.5 py-2 text-xs leading-relaxed text-[rgb(var(--muted))]">
              Looking for <b className="font-mono">{configured}</b>
              {state.data.match ? (
                <>
                  {' '}
                  — matched <b className="font-mono">{state.data.match.name}</b> v
                  {state.data.match.version || 'unknown'}.
                </>
              ) : (
                <> — no entry matches. Click the correct program below to use its exact name everywhere.</>
              )}
              {state.data.source === 'registry-backup' &&
                ' Read from the registry backup, so it may be slightly out of date.'}
            </p>
            ...
            <div className="max-h-[46vh] space-y-1 overflow-y-auto pr-1">
              {rows.map((program, index) => {
                ...
                return (
                  ...
                  >
                    <PackageSearch
                      size={13}
                      className={cn('shrink-0', isMatch ? 'text-nord-14' : 'text-[rgb(var(--muted))]')}
                    />
                    <span className="min-w-0 flex-1">
                      <span className="block truncate text-xs font-bold text-[rgb(var(--text))]">
                        {program.name}
                      </span>
                      {program.publisher && (
                    ...
                    <span className="shrink-0 font-mono text-2xs font-bold text-[rgb(var(--muted))]">
                      {program.version || '—'}
                    </span>
                  </button>
                )
              ...
              {rows.length === 0 && (
                <p className="py-6 text-center text-2xs text-[rgb(var(--muted))]">
