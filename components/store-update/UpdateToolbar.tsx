        ...
        <div className="flex min-w-0 flex-1 items-center gap-2.5 rounded-xl border bg-[rgb(var(--surface)/.6)] p-2.5 md:border-0 md:bg-transparent md:p-0">
          ...
          <div className="min-w-0 flex-1">
            {file ? (
              <>
                ...
                <div className="truncate font-mono text-2xs text-[rgb(var(--muted))]" title={file.path}>
                  {file.path}
                </div>
              </>
            ) : (
              <>
                ...
                <div className="truncate text-2xs text-[rgb(var(--muted))]">
                  Dest: <span className="font-mono">{settings?.store_update_path || '…'}</span>
                </div>
              </>
            )}
          </div>
          {file && (
            <button
              type="button"
              onClick={onClearFile}
              className="grid h-8 w-8 place-items-center rounded-lg hover:bg-[rgb(var(--border)/.5)]"
            >
              <X size={14} />
            </button>
          )}
        ...
        <div className="grid grid-cols-2 gap-2 md:flex md:flex-wrap md:items-center">
          ...
          <Button
            ...
            disabled={
              anyDeployRunning ||
              !checkoutCount ||
              (Object.values(versions) as any[]).some((v: any) => v.state === 'checking')
            }
          ...
          <Button
            size="sm"
            onClick={onDeployAll}
            disabled={anyDeployRunning || !file || !checkoutCount}
            className="col-span-2 md:col-span-1"
          >
