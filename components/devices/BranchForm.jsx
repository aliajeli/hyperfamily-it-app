...
export default function BranchForm({ value, onSubmit, saving }) {
  ...
  const field = (name, label, placeholder, props = {}) => {
    ...
    return <div><Label htmlFor={inputId}>{label}</Label><Input id={inputId} placeholder={placeholder} aria-required={name === 'name' || name === 'code'} {...register(name)} {...props} />{errors[name] && <p className="mt-1 text-[11px] text-nord-11">{errors[name].message}</p>}</div>
  }
  return <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
    <div className="grid gap-4 sm:grid-cols-2">{field('name', 'Name', 'Central Berlin')}{field('code', 'Code', 'BER-01')}</div>
