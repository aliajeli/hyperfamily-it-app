...
function titleFor(device) {
  ...
  if (device.device_type === 'POS' && device.checkout_number)
    return `POS · Checkout ${device.checkout_number}`
      ...
      {devices.map((device, index) => {
        ...
        return (
          ...
          >
            <span
              aria-hidden="true"
              className="absolute -right-9 -top-10 h-20 w-20 rounded-full bg-[rgb(var(--primary)/.07)] blur-2xl transition-transform duration-500 group-hover:scale-150"
            />
            <div className="relative flex min-w-0 items-start gap-1.5">
              <div className="grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-[rgb(var(--primary)/.11)] text-[rgb(var(--primary))] ring-1 ring-[rgb(var(--primary)/.12)]">
                <Icon size={13} />
              </div>
              <div className="min-w-0 flex-1">
                <div className="flex min-w-0 items-start justify-between gap-1">
                  <div className="min-w-0">
                    <p className="text-2xs font-extrabold uppercase tracking-[0.12em] text-[rgb(var(--muted))]">
                      {DEVICE_TYPE_DETAILS[device.device_type]?.label || device.device_type}
                    </p>
                    <h3 className="truncate text-xs font-black tracking-[0.02em]" title={titleFor(device)}>
                      {titleFor(device)}
                    </h3>
                  </div>
                  <Badge
                    status={device.status || 'unknown'}
                    className="shrink-0 gap-1 px-1 py-0 text-2xs capitalize"
                  >
                    {device.status || 'unknown'}
                  </Badge>
                </div>
                <p className="mt-0.5 truncate font-mono text-2xs font-bold text-[rgb(var(--primary))]">
                  {device.ip}
            ...
            <div className="relative mt-1 flex min-h-3.5 flex-wrap gap-1">
              {details.map((detail) => (
                <span
                  key={detail}
                  className="max-w-full truncate rounded-md bg-[rgb(var(--canvas)/.8)] px-1.5 py-0.5 text-2xs font-semibold text-[rgb(var(--muted))]"
                >
                  {detail}
                </span>
              ))}
              {device.device_type === 'Switch' && (
                <span className="rounded-md bg-nord-8/12 px-1.5 py-0.5 text-2xs font-extrabold text-nord-10">
                  {ports} ports
                </span>
              )}
              {!details.length && device.device_type !== 'Switch' && (
                <span className="text-2xs text-[rgb(var(--muted))]">No optional details</span>
              )}
            </div>
            ...
            <div className="relative mt-1 flex items-center justify-between border-t pt-0.5">
              <label className="flex min-w-0 items-center gap-1.5 text-2xs font-bold text-[rgb(var(--muted))]">
                ...
                />
                <span className={device.is_dashboard_visible ? 'status-online-text' : ''}>
                  {dashboardSavingId === device.id
                    ? 'Saving…'
                    : device.is_dashboard_visible
                      ? 'Dashboard on'
                      : 'Dashboard off'}
                </span>
              </label>
              <div className="flex gap-0.5">
                <Button
                  ...
                  aria-label={`Edit ${titleFor(device)}`}
                  title="Edit device"
                >
                  <Pencil size={10} />
                </Button>
                <Button
                  ...
                  variant="ghost"
                  size="icon"
                  className="h-5 w-5 text-nord-11"
                  onClick={() => onDelete(device)}
                  aria-label={`Delete ${titleFor(device)}`}
                  title="Delete device"
                >
                  <Trash2 size={10} />
                </Button>
