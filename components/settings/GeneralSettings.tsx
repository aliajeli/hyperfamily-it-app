...
export default function GeneralSettings({ settings, onSaved }: any) {
  ...
  useEffect(() => {
    getApi()
      .auth.recoverStatus?.()
      .then((status) => setPinSet(Boolean(status?.pinSet)))
      .catch(() => {})
  }, [])
  ...
  const updateAccount = async (event) => {
    ...
    if (username.length < 3 || username.length > 64)
      return toast.error('Username must contain between 3 and 64 characters')
    if (account.newPassword && account.newPassword.length < 4)
      return toast.error('New password must contain at least 4 characters')
    ...
    if (wantsPin && account.recoveryPin !== account.confirmRecoveryPin)
      return toast.error('The recovery PINs do not match')
    if (wantsPin && !/^\d{4,8}$/.test(account.recoveryPin))
      return toast.error('The recovery PIN must contain 4 to 8 digits')
    ...
    try {
      ...
      toast.success(
        wantsPin ? 'Administrator account and recovery PIN updated' : 'Administrator account updated'
      )
    } catch (error) {
  ...
  return (
    <div className="grid gap-3 xl:grid-cols-2">
      <Card>
        <CardHeader className="p-3 pb-1.5">
          <div className="flex items-center gap-2.5">
            <div className="rounded-lg bg-nord-15/15 p-2 text-nord-15">
              <UserRoundCog size={16} />
            </div>
            <div>
              <CardTitle className="text-sm">Administrator account</CardTitle>
              <CardDescription className="mt-0.5 text-2xs leading-snug">
                Change the login username, password, or both. Your current password is always required.
              </CardDescription>
            </div>
          </div>
        </CardHeader>
          ...
          <form onSubmit={updateAccount} className="space-y-2.5">
            ...
            <div className="rounded-xl border bg-[rgb(var(--canvas)/.55)] p-2.5">
              <div className="mb-1.5 flex items-center gap-1.5">
                ...
                <span
                  className={`ml-auto rounded-full px-1.5 py-0.5 text-xs font-extrabold uppercase ${pinSet ? 'bg-nord-14/20 text-[#66834e]' : 'bg-nord-13/20 text-[#8b6e1c]'}`}
                >
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                <label className="min-w-0">
                  ...
                  <Input
                    ...
                    autoComplete="off"
                    maxLength={8}
                    placeholder={pinSet ? 'Leave blank to keep' : 'e.g. 4821'}
                    value={account.recoveryPin}
                    onChange={(event) =>
                      setAccount({ ...account, recoveryPin: event.target.value.replace(/\D/g, '') })
                    }
                  />
                </label>
                <label className="min-w-0">
                  ...
                  <Input
                    ...
                    autoComplete="off"
                    maxLength={8}
                    value={account.confirmRecoveryPin}
                    onChange={(event) =>
                      setAccount({ ...account, confirmRecoveryPin: event.target.value.replace(/\D/g, '') })
                    }
                  />
                </label>
              </div>
              <p className="mt-1.5 text-xs leading-snug text-[rgb(var(--muted))]">
                {pinSet
                  ? 'A PIN is already set — fill both fields to replace it. It gates the Recover credentials option on the login screen; 5 wrong attempts lock recovery for 5 minutes.'
                  : 'Setting a PIN enables the Recover credentials option on the login screen. Keep it somewhere safe.'}
              </p>
            </div>

            <p className="text-xs leading-snug text-[rgb(var(--muted))]">
              The username is required at the next login. Leave the password blank to keep the current one; a
              longer passphrase is recommended.
            </p>
            <Button disabled={busy === 'account'}>
              {busy === 'account' ? 'Updating account…' : 'Update account'}
            </Button>
          </form>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader className="p-3 pb-1.5">
          <div className="flex items-center gap-2.5">
            <div className="rounded-lg bg-nord-8/15 p-2 text-nord-10">
              <Activity size={16} />
            </div>
            <div>
              <CardTitle className="text-sm">Real-time monitoring</CardTitle>
              <CardDescription className="mt-0.5 text-2xs leading-snug">
                Balance freshness with traffic across monitored networks.
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        <CardContent className="p-3 pt-1.5">
          <div className="mb-2.5 flex gap-2 rounded-lg border border-nord-14/35 bg-nord-14/10 p-2 text-xs leading-snug">
            ...
            <p>
              <b>Healthy threshold:</b> Ping responses up to and including 300 ms are classified as online.
              Higher responses show a warning.
            </p>
          </div>
            ...
            <div className="grid gap-2.5 sm:grid-cols-2">
              <label className="min-w-0">
                ...
                <Input
                  ...
                  min={1}
                  max={60}
                  value={ping.ping_interval}
                  onChange={(event) => setPing({ ...ping, ping_interval: event.target.value })}
                />
                <p className="mt-0.5 text-xs leading-snug text-[rgb(var(--muted))]">
              ...
              <label className="min-w-0">
                ...
                <Input
                  ...
                  min={10}
                  max={100}
                  value={ping.ping_history_count}
                  onChange={(event) => setPing({ ...ping, ping_history_count: event.target.value })}
                />
                <p className="mt-0.5 text-xs leading-snug text-[rgb(var(--muted))]">
                  10–100 responses. Default: 30.
                </p>
              </label>
            </div>
            <Button disabled={busy === 'ping'}>
              {busy === 'ping' ? 'Saving…' : 'Save monitoring settings'}
            </Button>
          </form>
        </CardContent>
      </Card>
