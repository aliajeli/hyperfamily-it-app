...
export default function CredentialsSettings() {
  ...
  const [usage, setUsage] = useState<any>({ devices: {}, types: {} })
  ...
  const load = useCallback(async () => {
    try {
      ...
      for (const row of overview)
        if (row.credential_id) devices[row.credential_id] = (devices[row.credential_id] || 0) + 1
      const types: any = {}
      for (const ids of Object.values(map?.types || {}) as any[])
        if (ids?.length) types[ids[0]] = (types[ids[0]] || 0) + 1
    ...
    <div className="grid gap-3 xl:grid-cols-[320px_1fr]">
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-2xs">
            Passwords are encrypted with Windows DPAPI before SQLite storage.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={add} className="space-y-2">
            <label className="block">
              ...
              <Input
                required
                placeholder="Branch administrators"
                value={form.name}
      ...
      <Card>
        <CardHeader className="pb-2">
          ...
          <CardDescription className="text-2xs">
            Revealed passwords hide again after 15 seconds. Assign them to equipment in the <b>Assignments</b>{' '}
            tab.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {credentials.length ? (
                ...
                <tbody>
                  {credentials.map((credential) => {
                    ...
                    return (
                      <tr
                        key={credential.id}
                        className="border-b transition last:border-0 hover:bg-[rgb(var(--border)/.3)]"
                      >
                        ...
                        <td className="py-1.5 text-xs text-[rgb(var(--muted))]">
                          {deviceCount || typeCount
                            ? [
                                ...
                                typeCount ? `${typeCount} type default${typeCount === 1 ? '' : 's'}` : null
                              ]
                                .filter(Boolean)
                                .join(' · ')
                            : 'Not assigned'}
                        ...
                        <td className="py-1.5">
                          <div className="flex justify-end gap-1">
                            <Button
                              ...
                              aria-label={`${revealed[credential.id] ? 'Hide' : 'Reveal'} password for ${credential.name}`}
                            >
                              {revealed[credential.id] ? <EyeOff size={14} /> : <Eye size={14} />}
                            </Button>
                            <Button
                              variant="ghost"
                              size="icon"
                              className="h-7 w-7 text-nord-11"
            ...
            <EmptyState
              ...
              description="Create an encrypted credential to enable one-click remote connections."
            />
          )}
        </CardContent>
      </Card>
