...
export default function Header({ collapsed, user }) {
  ...
  return <header className={`drag-region fixed right-0 top-0 z-20 flex h-16 items-center justify-between border-b bg-[rgb(var(--canvas)/.76)] px-4 backdrop-blur-xl transition-all duration-300 md:px-6 ${collapsed ? 'md:left-[84px]' : 'md:left-64'}`}>
    ...
    <div className="no-drag flex items-center gap-2.5">
      <div className="group hidden h-10 w-64 items-center gap-2 rounded-xl border bg-[rgb(var(--surface)/.58)] px-3 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-[rgb(var(--primary)/.35)] hover:bg-[rgb(var(--surface)/.8)] hover:shadow-md focus-within:border-[rgb(var(--focus))] xl:flex"><Search size={16} className="text-[rgb(var(--muted))] transition-transform duration-300 group-focus-within:scale-110 group-focus-within:text-[rgb(var(--primary))]" /><input aria-label="Global search" placeholder="Search devices…" className="w-full bg-transparent text-xs outline-none" /><span className="flex items-center gap-1 rounded-md border px-1.5 py-0.5 text-[9px] text-[rgb(var(--muted))]"><Command size={9} />K</span></div>
      <VPNButton />
      <NotificationCenter />
