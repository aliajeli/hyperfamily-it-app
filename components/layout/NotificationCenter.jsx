...
const summaryMeta = [
  {
    ...
    label: 'Offline',
    icon: WifiOff,
    className: 'border-nord-11/35 bg-nord-11/10 text-nord-11'
  },
  {
    key: 'warning',
    label: 'Warning',
    icon: TriangleAlert,
    className: 'border-nord-13/40 bg-nord-13/12 status-warning-text'
  },
  {
    key: 'online',
    label: 'Online',
    icon: CheckCircle2,
    className: 'border-nord-14/35 bg-nord-14/10 status-online-text'
  },
  {
    key: 'branches',
    label: 'Branches',
    icon: Building2,
    className: 'border-nord-8/35 bg-nord-8/10 text-nord-10'
  }
]
      ...
      >
        ...
        {totalCount > 0 ? (
          <motion.span
            initial={{ scale: 0 }}
            animate={{ scale: 1 }}
            className="notification-count absolute -right-1.5 -top-1.5 grid min-h-4 min-w-4 place-items-center rounded-full bg-nord-11 px-1 text-2xs font-black leading-none text-white shadow-md ring-2 ring-[rgb(var(--canvas))]"
          >
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <span
              aria-hidden="true"
              className="absolute -right-14 -top-16 h-36 w-36 rounded-full bg-[rgb(var(--primary)/.06)] blur-3xl"
            />
            <header className="relative flex items-start justify-between border-b px-4 py-3.5">
              <div>
                <h2 className="flex items-center gap-2 text-sm font-black tracking-[0.025em]">
                  <Bell size={15} className="text-[rgb(var(--primary))]" /> Network notifications
                </h2>
              ...
              <button
                ...
                className="grid h-7 w-7 place-items-center rounded-lg text-[rgb(var(--muted))] transition-all hover:rotate-90 hover:bg-[rgb(var(--border)/.55)] hover:text-[rgb(var(--text))]"
              >
                <X size={14} />
              </button>
            </header>
            ...
            <div className="relative p-3.5">
              <div className="grid grid-cols-4 gap-1.5" aria-label="Network summary">
                {summaryMeta.map(({ key, label, icon: Icon, className }, index) => (
                  <motion.div
                    ...
                    className={`min-w-0 rounded-xl border p-2 ${className}`}
                  >
                    <div className="flex items-center justify-between gap-1">
                      <Icon size={12} />
                      <span className="text-sm font-black tabular-nums">
                        {generatedAt ? model.stats[key] : '—'}
                      </span>
                    </div>
                    <p className="mt-1 truncate text-2xs font-extrabold uppercase tracking-[0.1em]">
                      {label}
                    </p>
                  </motion.div>
                ))}
              ...
              {updateAvailable && (
                ...
                >
                  <div className="flex items-center gap-2">
                    <span className="grid h-8 w-8 shrink-0 place-items-center rounded-[10px] bg-[rgb(var(--primary-strong))] text-white shadow-md">
                      <Rocket size={14} />
                    </span>
                    <div className="min-w-0 flex-1">
                      <p className="flex items-center gap-1.5 text-xs font-extrabold tracking-[0.03em]">
                        Update available
                        <span className="rounded-full bg-[rgb(var(--primary)/.16)] px-1.5 py-0.5 text-2xs font-black text-[rgb(var(--primary))]">
                  ...
                  <div className="mt-2 flex gap-1.5">
                    <button
                      ...
                      className="flex h-7 items-center gap-1.5 rounded-lg bg-[rgb(var(--primary-strong))] px-2.5 text-xs font-extrabold text-white shadow-sm transition hover:brightness-110"
                    >
              ...
              <div className="mb-2 mt-4 flex items-center justify-between px-0.5">
                <p className="flex items-center gap-1.5 text-xs font-extrabold uppercase tracking-[0.14em] text-[rgb(var(--muted))]">
                  <Activity size={11} /> Active alerts
                </p>
              ...
              {alertCount ? (
                  ...
                  {model.alerts.map((device, index) => {
                    ...
                    return (
                      <motion.article
                        ...
                        className="group flex min-w-0 items-center gap-2.5 rounded-xl border bg-[rgb(var(--surface)/.82)] p-2.5 transition-all hover:-translate-y-0.5 hover:bg-[rgb(var(--surface))] hover:shadow-md"
                      >
                        <div
                          className={`grid h-8 w-8 shrink-0 place-items-center rounded-[10px] ${device.status === 'offline' ? 'bg-nord-11/12 text-nord-11' : 'bg-nord-13/15 status-warning-text'}`}
                        >
                          <Icon size={14} />
                        </div>
                        <div className="min-w-0 flex-1">
                          <p
                            className="truncate text-xs font-extrabold tracking-[0.03em]"
                            title={device.name}
                          >
                            {device.name}
                          </p>
                          <p className="truncate text-2xs font-semibold text-[rgb(var(--muted))]">
                            {device.branch?.name || 'Unknown branch'} · {device.ip_address || device.ip}
                          </p>
                          <p className="mt-0.5 flex items-center gap-1 text-2xs text-[rgb(var(--muted))]">
                            <Clock3 size={8} /> Last check {latestCheck(device)}
                          </p>
                        </div>
                        <div className="shrink-0 text-right">
                          <DeviceStatusBadge status={device.status} compact />
                          <p className="mt-1 text-2xs font-black tabular-nums text-[rgb(var(--muted))]">
                            {Number.isFinite(ping) ? `${ping} ms` : 'No reply'}
                          </p>
                        </div>
                      </motion.article>
                    )
                ...
                <motion.div
                  ...
                  className="grid min-h-32 place-items-center rounded-2xl border border-nord-14/30 bg-nord-14/8 p-5 text-center"
                >
                  <div>
                    <motion.div
                      animate={{ scale: [1, 1.08, 1] }}
                      transition={{ duration: 2.2, repeat: Infinity }}
                      className="mx-auto grid h-10 w-10 place-items-center rounded-full bg-nord-14/16 status-online-text"
                    >
                      <CheckCircle2 size={20} />
                    </motion.div>
                    <p className="mt-2 text-xs font-black">All monitored devices are healthy</p>
                    <p className="mt-1 text-xs text-[rgb(var(--muted))]">
                      No active Offline or Warning notifications.
                    </p>
                  </div>
                </motion.div>
              )}
