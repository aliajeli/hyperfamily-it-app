...
const SECTIONS = [
  {
    ...
    chip: 'bg-nord-14/14 text-[#5c7a46] dark:text-nord-14'
  },
  {
    key: 'improved',
    label: 'Improved',
    ...
    chip: 'bg-[rgb(var(--primary)/.12)] text-[rgb(var(--primary))]'
  },
  { key: 'fixed', label: 'Fixed', icon: Bug, className: 'text-nord-12', chip: 'bg-nord-12/14 text-nord-12' },
  {
    ...
    chip: 'bg-[rgb(var(--border)/.6)] text-[rgb(var(--muted))]'
  },
  {
    key: 'security',
    label: 'Security',
    ...
    chip: 'bg-nord-13/16 status-warning-text'
  }
]

/** Drops sections a version does not use, in a stable display order. */
function sectionsOf(entry) {
  return SECTIONS.map((section) => ({ ...section, items: entry?.changes?.[section.key] || [] })).filter(
 ...
 */
function VersionNotes({ entry, compact = false }: any) {
  if (!entry) {
    return (
      <p className="text-2xs text-[rgb(var(--muted))]">
        No changelog is bundled for this version. The release notes of every published build are on the GitHub
        releases page.
      </p>
    )
  }
        ...
        {entry.channel && (
          <span
            className={cn(
              ...
              entry.channel === 'beta'
                ? 'bg-nord-13/18 status-warning-text'
                : 'bg-nord-14/14 status-online-text'
            )}
          >
        ...
        <div className="space-y-1.5">
          {sections.map(({ key, label, icon: Icon, className, chip, items }) => (
            <div key={key}>
              <p
                className={cn(
                  'mb-0.5 flex items-center gap-1.5 text-xs font-extrabold uppercase tracking-[0.14em]',
                  className
                )}
              >
                <Icon size={11} />
                ...
                <span
                  className={cn(
                    'ml-0.5 rounded-full px-1.5 py-px text-xs font-bold normal-case tracking-normal',
                    chip
                  )}
                >
                  {items.length}
              ...
              <ul className={cn('space-y-0.5 pl-[17px]', compact ? 'text-xs' : 'text-2xs')}>
                {items.map((item) => (
                  <li
                    key={item}
                    className="relative leading-relaxed text-[rgb(var(--muted))] before:absolute before:-left-3 before:top-[0.55em] before:h-1 before:w-1 before:rounded-full before:bg-[rgb(var(--muted)/.55)] before:content-['']"
                  >
                    {item}
                  </li>
                ))}
...
export default function ChangeLogCard({
  ...
  const current = useMemo(
    () => entries.find((entry) => entry.version === version) || null,
    [entries, version]
  )
  const pending = useMemo(
    () => entries.find((entry) => entry.version === updateVersion) || null,
    [entries, updateVersion]
  )
        ...
        {pending && pending.version !== version && (
          <div className="mb-2 rounded-xl border border-[rgb(var(--primary)/.35)] bg-[rgb(var(--primary)/.06)] p-2.5">
            <p className="mb-1.5 flex items-center gap-1.5 text-xs font-extrabold uppercase tracking-[0.14em] text-[rgb(var(--primary))]">
              <Sparkles size={12} />
              Coming in the available update
            </p>
            {/* The bundled entry wins; a remote note is the fallback for a
                version published after this build. */}
            {pending ? (
              <VersionNotes entry={pending} compact />
            ) : (
              <p className="whitespace-pre-line text-xs leading-relaxed text-[rgb(var(--muted))]">
                {updateNotes || 'Release notes are published with the update.'}
              </p>
            )}
          </div>
        )}
            ...
            >
              <div className="mt-2 max-h-[46vh] space-y-2 overflow-y-auto pr-1">
                ...
                {history.map((entry) => (
                  <details
                    key={entry.version}
                    className="group rounded-xl border bg-[rgb(var(--surface)/.38)] px-2.5 py-2"
                  >
                    <summary className="flex cursor-pointer list-none items-center gap-2 text-2xs font-bold text-[rgb(var(--text))] marker:content-none">
                      <ChevronDown
                        size={13}
                        className="shrink-0 text-[rgb(var(--muted))] transition-transform group-open:rotate-180"
                      />
                      <span className="font-mono text-[rgb(var(--primary))]">v{entry.version}</span>
                      <span className="min-w-0 flex-1 truncate font-sans font-semibold text-[rgb(var(--muted))]">
                        {entry.title}
                      </span>
                      <span className="shrink-0 font-mono text-xs font-semibold text-[rgb(var(--muted))]">
                        {entry.date}
                      </span>
                    </summary>
