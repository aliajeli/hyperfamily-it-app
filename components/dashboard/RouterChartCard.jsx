...
export default function RouterChartCard({ branch, gateway, expanded = false }) {
  ...
  return (
    <motion.div
      ...
      className={`router-monitor-card group relative overflow-hidden rounded-2xl border shadow-sm ${gatewayTreatment[gatewayStatus] || gatewayTreatment.unknown}`}
    >
      ...
      <button type="button" onClick={openGatewayDetails} className={`interactive-sheen relative block w-full text-left ${expanded ? 'p-4' : 'p-2.5'}`} aria-label={`Open detailed Router chart for ${branch.name}`}>
        ...
        <div className={`${expanded ? 'mt-3 px-2' : 'mt-1.5 px-1'} overflow-hidden rounded-xl bg-[rgb(var(--surface)/.58)] ring-1 ring-[rgb(var(--border)/.35)]`}>
