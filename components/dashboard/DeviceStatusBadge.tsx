...
export default function DeviceStatusBadge({ status = 'unknown', compact = false }: any) {
  const current = meta[status] || meta.unknown
  const Icon = current.icon

  return (
    <span
      className={`inline-flex shrink-0 items-center rounded-full border font-extrabold uppercase tracking-[0.08em] ${compact ? 'gap-1 px-1.5 py-0.5 text-2xs' : 'gap-1.5 px-2 py-1 text-2xs'} ${current.className}`}
    >
