...
export default function BranchDetailsPanel({ branch, devices, view = 'modal', onClose }: any) {
    ...
    >
      <DialogPrimitive.Portal>
        ...
        >
          <span
            aria-hidden="true"
            className="pointer-events-none absolute -left-24 -top-28 h-72 w-72 rounded-full bg-[rgb(var(--primary)/.13)] blur-3xl"
          />
          <span
            aria-hidden="true"
            className="pointer-events-none absolute -bottom-28 right-0 h-72 w-72 rounded-full bg-[rgb(var(--secondary)/.11)] blur-3xl"
          />
          ...
          <div className={`relative flex h-full flex-col ${sidePanel ? 'max-h-screen' : 'max-h-[92vh]'}`}>
            <header className="shrink-0 border-b bg-[rgb(var(--surface)/.44)] px-5 py-4 backdrop-blur-xl sm:px-6">
              <div className="flex min-w-0 items-start gap-3 pr-11">
                <motion.div
                  ...
                  className="grid h-11 w-11 shrink-0 place-items-center rounded-2xl bg-[rgb(var(--primary)/.14)] text-[rgb(var(--primary))] ring-1 ring-[rgb(var(--primary)/.15)]"
                >
                  <MapPin size={20} />
                </motion.div>
                <div className="min-w-0 flex-1">
                  <div className="flex min-w-0 flex-wrap items-center gap-2">
                    <DialogPrimitive.Title className="truncate text-lg font-black tracking-[0.035em] sm:text-xl">
                      {branch.name}
                    </DialogPrimitive.Title>
                    <span className="rounded-lg bg-[rgb(var(--border)/.55)] px-2 py-1 font-mono text-xs font-extrabold tracking-wider">
                      {branch.code}
                    </span>
                  </div>
              ...
              <div className="mt-3 grid grid-cols-3 gap-2">
                {summaryMeta.map(({ key, label, icon: Icon, className }, index) => (
                  <motion.div
                    ...
                    className={`flex items-center gap-2 rounded-xl border px-2.5 py-2 ${className}`}
                  >
                    <Icon size={14} className="shrink-0" />
                    <span className="min-w-0">
                      <span className="block text-sm font-black leading-none tabular-nums">
                        {summary[key]}
                      </span>
                      <span className="mt-0.5 block truncate text-2xs font-extrabold uppercase tracking-[0.13em]">
                        {label}
                      </span>
                    </span>
                  </motion.div>
            ...
            <DialogPrimitive.Close asChild>
              <button
                ...
                className="group absolute right-4 top-4 z-10 grid h-9 w-9 place-items-center rounded-xl border bg-[rgb(var(--surface)/.72)] text-[rgb(var(--muted))] shadow-sm transition-all duration-300 hover:rotate-90 hover:scale-105 hover:bg-[rgb(var(--surface))] hover:text-[rgb(var(--text))]"
              >
                <X size={17} />
              </button>
              ...
              <motion.div variants={listVariants} initial="hidden" animate="visible" className="space-y-5">
                ...
                <motion.section variants={itemVariants}>
                  <div className="mb-2.5 flex items-center justify-between px-0.5">
                    <div>
                      <h3 className="flex items-center gap-2 text-xs font-black tracking-[0.04em]">
                        <Building2 size={14} className="text-[rgb(var(--primary))]" /> Monitored equipment
                      </h3>
                      <p className="mt-0.5 text-xs text-[rgb(var(--muted))]">
                        Visibility is controlled by each device&apos;s Dashboard setting.
                      </p>
                    </div>
                    <span className="rounded-full border bg-[rgb(var(--surface)/.7)] px-2.5 py-1 text-xs font-black tabular-nums">
                      {equipment.length}
                    </span>
                  </div>
                  ...
                  {equipment.length ? (
                    <div
                      className={`grid gap-2 ${sidePanel ? 'grid-cols-1 sm:grid-cols-2' : 'sm:grid-cols-2 lg:grid-cols-3'}`}
                    >
