  ...
  return (
    <aside className="flex h-full min-h-0 w-full flex-col gap-2.5 rounded-2xl border bg-[rgb(var(--surface))] p-3">
      <div className="flex items-center gap-2">
        ...
        <span className="ml-auto rounded-md bg-[rgb(var(--border)/.6)] px-1.5 py-0.5 text-xs font-bold">{snippets.length}</span>
      </div>

      <div className="relative">
        <Search size={13} className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-[rgb(var(--muted))]" />
        <Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Filter snippets" className="pl-8 text-2xs" aria-label="Filter snippets" />
      </div>
              ...
              <div className="flex items-start gap-1.5">
                ...
                >
                  <div className="flex items-center gap-1.5">
                    ...
                    {lines.length > 1 && <span className="shrink-0 rounded bg-[rgb(var(--primary)/.16)] px-1 py-px text-xs font-bold text-[rgb(var(--primary))]">{lines.length} lines</span>}
                  </div>
                  <div className="truncate font-mono text-xs text-[rgb(var(--primary))]">{lines[0] || snippet.command}</div>
                  {lines.length > 1 && <div className="truncate font-mono text-xs text-[rgb(var(--muted))]">{lines[1]}{lines.length > 2 ? ' …' : ''}</div>}
                  {snippet.description && <div className="truncate text-xs text-[rgb(var(--muted))]">{snippet.description}</div>}
                </button>
        ...
        {draft ? (
          ...
          >
            <div className="flex items-center justify-between">
              <p className="text-xs font-extrabold uppercase tracking-wider text-[rgb(var(--muted))]">{draft.id ? 'Edit snippet' : 'New snippet'}</p>
            ...
            <div>
              ...
              <Input id="snippet-name" value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} placeholder="Show interfaces" className="text-2xs" autoFocus />
            </div>
            <div>
            ...
            <div>
              ...
              <Input id="snippet-description" value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} placeholder="Optional note" className="text-2xs" />
            </div>
