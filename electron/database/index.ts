...
class AppDatabase {
  ...
  static sanitiseTags(value) {
    ...
    return [
      ...new Set(
        value.map((tag) => String(tag).replace(/^#+/, '').trim().toLowerCase().slice(0, 40)).filter(Boolean)
      )
    ].slice(0, 20)
  }
  ...
  syncRecoveryFile() {
    try {
      const admin = this.db
        .prepare('SELECT username, password_recovery, recovery_pin_hash FROM users ORDER BY id LIMIT 1')
        .get()
      ...
      /* best-effort mirror; the tool explains itself if the file is stale */
    }
  }

  /** Decrypts the current recovery file, or null when absent/unreadable. */
  readRecoveryFile() {
    const candidates = [path.join(this.userDataPath, 'credentials.dat'), this.recoveryFilePath].filter(
      Boolean
    )
  ...
  writeRecoveryFile(payload) {
    ...
    const targets = new Set(
      [path.join(this.userDataPath, 'credentials.dat'), this.recoveryFilePath].filter(Boolean)
    )
  ...
  saveRememberedCredentials(payload) {
    ...
    try {
      fs.writeFileSync(
        this.rememberedCredentialsPath(),
        this.vault.encrypt(JSON.stringify({ username, password })),
        { mode: 0o600 }
      )
      return { saved: true }
    } catch {
  ...
  clearRememberedCredentials() {
    try {
      fs.unlinkSync(this.rememberedCredentialsPath())
    } catch {
      /* nothing stored */
    }
    return { saved: false }
  ...
  setRecoveryPin(userId, pin) {
    ...
    this.db
      .prepare('UPDATE users SET recovery_pin_hash = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
      .run(AppDatabase.hashPin(value), Number(userId))
    this.audit(
      this.db.prepare('SELECT username FROM users WHERE id = ?').get(Number(userId))?.username || 'Admin',
      'RECOVERY_PIN',
      'set',
      'Credential recovery PIN updated'
    )
    this.syncRecoveryFile()
    return { success: true }
  }
  ...
  encryptLegacyDatabase(databaseKey) {
    ...
    try {
      ...
      if (this.hasPlaintextHeader(temporary))
        throw new Error('Encrypted migration output still has a plaintext SQLite header')

      for (const suffix of ['-wal', '-shm']) {
        try {
          fs.unlinkSync(`${this.filePath}${suffix}`)
        } catch (error) {
          if (error.code !== 'ENOENT') throw error
        }
      }
      ...
      if (sourceMoved && !fs.existsSync(this.filePath) && fs.existsSync(backup))
        fs.renameSync(backup, this.filePath)
      this.removeDatabaseFiles(temporary)
  ...
  updateCredentials(userId, payload: any = {}) {
    ...
    try {
      this.db
        .prepare(
          'UPDATE users SET username = ?, password = ?, password_recovery = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?'
        )
    ...
    this.audit(
      ...
      'ACCOUNT_UPDATE',
      nextUsername,
      `Administrator ${changed.length ? changed.join(' and ') : 'credentials verified'}`
    )
    this.syncRecoveryFile()
    ...
    try {
      ...
      if (remembered.username) {
        this.saveRememberedCredentials({
          ...
          password: nextPassword || remembered.password
        })
      }
    } catch {
  ...
  listBranches() {
    return this.db.prepare('SELECT * FROM branches ORDER BY name COLLATE NOCASE').all()
  }
  ...
  saveBranch(data, actor = 'Admin') {
    ...
    if (!/^[A-Za-z0-9_-]+$/.test(warehouseCode) || warehouseCode.length > 40)
      throw new Error('Warehouse Code must use no more than 40 letters, numbers, dashes, or underscores')
    const values = BRANCH_COLUMNS.map((key) => String(data[key] || '').trim() || null)
    let result
    if (data.id) {
      const assignments = BRANCH_COLUMNS.map((key) => `${key} = ?`).join(', ')
      result = this.db
        .prepare(`UPDATE branches SET ${assignments}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
        .run(...values, Number(data.id))
    ...
    result = this.db
      .prepare(
        `INSERT INTO branches (${BRANCH_COLUMNS.join(',')}) VALUES (${BRANCH_COLUMNS.map(() => '?').join(',')})`
      )
      .run(...values)
  ...
  deleteAllBranchesAndDevices(actor = 'Admin') {
    ...
    this.audit(
      ...
      'DIRECTORY_CLEAR',
      'all',
      `${counts.branchCount} branches and ${counts.deviceCount} devices permanently removed`
    )
  ...
  listSwitchPorts(deviceId = null) {
    if (deviceId)
      return this.db
        .prepare('SELECT * FROM switch_ports WHERE device_id = ? ORDER BY port_number')
        .all(Number(deviceId))
  ...
  listMonitoredDevices() {
    return this.db.prepare('SELECT * FROM devices WHERE is_dashboard_visible = 1 ORDER BY id').all()
  }
  ...
  replaceSwitchPorts(deviceId, ports = []) {
    ...
    const insert = this.db
      .prepare(`INSERT INTO switch_ports (device_id, port_number, vlan, status, ip, details)
      VALUES (?, ?, ?, ?, ?, ?)`)
  ...
  saveDevice(data, actor = 'Admin') {
    ...
    const switchPorts =
      normalized.device_type === 'Switch' ? normalizeSwitchPorts(data.switch_ports || []) : []
    const values = DEVICE_COLUMNS.map((key) =>
      normalized[key] === '' || normalized[key] === undefined ? null : normalized[key]
    )

    return this.db.transaction(() => {
      if (normalized.device_type === 'Router') {
        const deviceId = Number(data.id) || 0
        const existingRouter = this.db
          .prepare(
        ...
        const current = deviceId
          ? this.db.prepare('SELECT branch_id, device_type FROM devices WHERE id = ?').get(deviceId)
          : null
        const editingLegacyRouterInPlace =
          current?.device_type === 'Router' && current.branch_id === normalized.branch_id
        if (existingRouter && !editingLegacyRouterInPlace)
      ...
      if (data.id) {
        ...
        const result = this.db
          .prepare(
            `UPDATE devices SET ${DEVICE_COLUMNS.map((key) => `${key} = ?`).join(', ')}, updated_at = CURRENT_TIMESTAMP WHERE id = ?`
          )
          .run(...values, deviceId)
        ...
        const result = this.db
          .prepare(
            `INSERT INTO devices (${DEVICE_COLUMNS.join(',')}) VALUES (${DEVICE_COLUMNS.map(() => '?').join(',')})`
          )
          .run(...values)
    ...
    return this.db.transaction(() => {
      ...
      for (const branch of branchRows) {
        const existing = this.db
          .prepare('SELECT id FROM branches WHERE code = ? COLLATE NOCASE')
          .get(branch.code)
      ...
      for (const device of deviceRows) {
        const branch = this.db
          .prepare('SELECT id FROM branches WHERE code = ? COLLATE NOCASE')
          .get(device.branch_code)
        if (!branch) throw new Error(`Branch Code "${device.branch_code}" does not exist`)

        const existing =
          device.device_type === 'Router'
            ? this.db
                .prepare("SELECT id FROM devices WHERE branch_id = ? AND device_type = 'Router' LIMIT 1")
                .get(branch.id)
            : this.db
                .prepare(
                  'SELECT id FROM devices WHERE branch_id = ? AND device_type = ? AND ip = ? COLLATE NOCASE LIMIT 1'
                )
                .get(branch.id, device.device_type, device.ip)
      ...
      this.audit(
        ...
        'DIRECTORY_IMPORT',
        'Excel workbook',
        `${branchChanges} branches, ${deviceChanges} devices, ${summary.switch_ports_imported} switch ports processed`
      )
  ...
  listCredentials() {
    return this.db
      .prepare(
        'SELECT id, name, username, created_at, 1 AS has_password FROM credentials ORDER BY name COLLATE NOCASE'
      )
      .all()
  }
  revealCredential(id) {
  ...
  saveCredential(data, actor = 'Admin') {
    if (!data.name?.trim() || !data.username?.trim() || !data.password)
      throw new Error('Name, username, and password are required')
    const result = this.db
      .prepare('INSERT INTO credentials (name, username, password) VALUES (?, ?, ?)')
      .run(data.name.trim(), data.username.trim(), this.vault.encrypt(data.password))
  ...
  getMappings() {
    ...
    for (const row of this.db
      .prepare('SELECT device_type, credential_id FROM device_credentials ORDER BY device_type')
      .all()) {
  ...
  saveMappings(mappings, actor = 'Admin') {
    ...
    const insertType = this.db.prepare(
      'INSERT OR IGNORE INTO device_credentials (device_type, credential_id) VALUES (?, ?)'
    )
    const clearDevices = this.db.prepare('DELETE FROM device_credential_assignments')
    const insertDevice = this.db.prepare(
      'INSERT OR IGNORE INTO device_credential_assignments (device_id, credential_id) VALUES (?, ?)'
    )
  ...
  setDeviceCredential(deviceId, credentialId, actor = 'Admin') {
    const device = this.db
      .prepare('SELECT id, name, device_type FROM devices WHERE id = ?')
      .get(Number(deviceId))
    if (!device) throw new Error('Device not found')

    const id =
      credentialId === null || credentialId === undefined || credentialId === '' ? null : Number(credentialId)
    if (id !== null) {
  ...
  setTypeCredential(deviceType, credentialId, actor = 'Admin') {
    ...
    if (id !== null) {
      ...
      this.db.transaction(() => {
        ...
        this.db
          .prepare('INSERT OR IGNORE INTO device_credentials (device_type, credential_id) VALUES (?, ?)')
          .run(type, id)
      })()
  ...
  getDeviceCredentialState(deviceId) {
    ...
    const inherited =
      ...
      WHERE t.device_type = ? LIMIT 1`
        )
        .get(device.device_type) || null
    return {
      device_id: device.id,
  ...
  listCredentialsForDevice(deviceId) {
    ...
    const rows = this.db
      ...
      .all(device.id, device.device_type)

    const seen = new Map()
    for (const row of rows.sort(
      (a, b) => (a.scope === 'device' ? -1 : 1) - (b.scope === 'device' ? -1 : 1)
    )) {
      if (!seen.has(row.id)) seen.set(row.id, row)
    }
    return [...seen.values()].sort((a, b) =>
      a.scope === b.scope ? a.name.localeCompare(b.name) : a.scope === 'device' ? -1 : 1
    )
  }
  ...
  listNotes() {
    ...
    return this.db
      .prepare(
        'SELECT id, name, body, pinned, color, priority, tags, created_at, updated_at FROM notes ORDER BY pinned DESC, priority DESC, updated_at DESC'
      )
      .all()
  ...
  saveNote(payload: any = {}, actor = 'System') {
    ...
    if (payload.id) {
      this.db
        .prepare(
          'UPDATE notes SET name = ?, body = ?, pinned = ?, color = ?, priority = ?, tags = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?'
        )
    ...
    const info = this.db
      .prepare('INSERT INTO notes (name, body, pinned, color, priority, tags) VALUES (?, ?, ?, ?, ?, ?)')
      .run(name, body, pinned, color, priority, tagsJson)
  ...
  saveSnippet(payload: any = {}, actor = 'System') {
    ...
    if (payload.id) {
      this.db
        .prepare(
          'UPDATE terminal_snippets SET name = ?, command = ?, description = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?'
        )
    ...
    const info = this.db
      .prepare('INSERT INTO terminal_snippets (name, command, description) VALUES (?, ?, ?)')
      .run(name, command, description)
  ...
  recordPingBatch(results) {
    const insertPing = this.db.prepare(
      'INSERT INTO ping_history (device_id, ping_time, status) VALUES (?, ?, ?)'
    )
    const updateUptime = this.db
      .prepare(`INSERT INTO uptime_logs (device_id, uptime_percent, total_checks, successful_checks, date)
  ...
  getMonitorSnapshot(historyCount = 30) {
    ...
    const devices = this.listDevices()
      .filter((device) => device.is_dashboard_visible)
      .map((device) => ({
        ...device,
        status: device.status || 'unknown',
        history: this.db
          .prepare(
            'SELECT ping_time, status, timestamp FROM ping_history WHERE device_id = ? ORDER BY id DESC LIMIT ?'
          )
          .all(device.id, Number(historyCount))
          .reverse()
  ...
  audit(user, action, target = null, details = null) {
    this.db
      .prepare('INSERT INTO audit_logs (user, action, target, details) VALUES (?, ?, ?, ?)')
      .run(user || 'System', action, target, details)
  }
  listAudit(limit = 200) {
    return this.db
      .prepare('SELECT * FROM audit_logs ORDER BY id DESC LIMIT ?')
      .all(Math.min(1000, Math.max(1, Number(limit))))
  }
}
