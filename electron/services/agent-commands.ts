      ...
      while (this.now() < deadline) {
        ...
        try {
          raw = await withTimeout(
            ...
            30000,
            `Reading the ${action} answer from ${host} stalled`
          )
        } catch (error) {
        ...
        if (raw) {
          if (Buffer.byteLength(raw) > MAX_RESULT_BYTES)
            throw new Error('The agent answer is larger than the supported limit')
          const result = JSON.parse(raw.toString('utf8').replace(/^\uFEFF/, ''))
          if (!result || result.protocolVersion !== 1 || result.id !== id)
            throw new Error('The agent returned an answer for a different command')
          await this.unlink(resultFile)
          await this.unlink(commandFile)
          return result
        }
        try {
          await this.delay(POLL_MS, undefined, { signal })
        } catch {
          if (signal?.aborted) throw cancelledError(`The ${action} command on ${host}`)
        }
      }
