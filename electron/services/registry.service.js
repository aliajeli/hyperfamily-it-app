...
function runReg(args, timeoutMs = 25000) {
  return new Promise((resolve) => {
    execFile(
      'reg.exe',
      args,
      { timeout: timeoutMs, windowsHide: true, encoding: 'utf8', maxBuffer: 32 * 1024 * 1024 },
      (error, stdout = '', stderr = '') => {
        resolve({
          ok: !error,
...
async function listRemotePrograms(host, options = {}) {
  ...
  const runs = await Promise.all(
    UNINSTALL_KEYS.map((key) => exec([`query`, `${prefix}HKLM\\${key}`, '/s'], timeoutMs))
  )
  const reachable = runs.some((run) => run.ok)
  if (!reachable) {
    const detail =
      runs.map((run) => (run.stderr || run.stdout || '').replace(/\s+/g, ' ').trim()).find(Boolean) || ''
    ...
    if (runs.some((run) => run.timedOut))
      fail(`${clean || 'This machine'} did not answer the registry query in time`, 'REGISTRY_TIMEOUT')
    if (/access is denied/i.test(detail))
      fail(
        `Access denied reading the registry on ${clean} — check Settings → Store App → Target access`,
        'REGISTRY_DENIED'
      )
    ...
    fail(
      `Registry query failed on ${clean || 'this machine'}${detail ? ` — ${detail}` : ''}`,
      'REGISTRY_FAILED'
    )
  }
  const seen = new Set()
...
async function readHiveOverShare(host, options = {}) {
  ...
  try {
    ...
    try {
      const runs = await Promise.all(
        UNINSTALL_KEYS.map((key) =>
          exec(
            ['query', `HKLM\\${mountName}\\${key.replace(/^SOFTWARE\\/i, '')}`, '/s'],
