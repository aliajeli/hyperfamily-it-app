...
function formatProgress({ bytes, totalBytes, elapsedMs, bytesPerSecond }) {
  ...
  const count =
    totalBytes > 0
      ? `${mb(bytes)} / ${mb(totalBytes)} MB (${Math.min(100, Math.floor((bytes * 100) / totalBytes))}%)`
      : `${mb(bytes)} MB`
...
async function runTransfer(task, options = {}) {
  ...
  if (![idleTimeoutMs, maxDurationMs].every((value) => Number.isFinite(value) && value > 0))
    throw new Error('Invalid agent transfer timeout')
  ...
  const statistics = () => {
    ...
    return {
      ...
      elapsedMs,
      bytesPerSecond: elapsedMs > 0 ? (bytes * 1000) / elapsedMs : 0
    }
  }
  const report = () => {
    try {
      options.onProgress?.(statistics())
    } catch {
      /* UI disposal must not interrupt file safety/cleanup. */
    }
  }
  const expire = (idle) => {
    if (failure) return
    ...
    const reason = idle
      ? `no I/O progress for ${idleTimeoutMs / 1000} seconds`
      : `exceeded the ${maxDurationMs / 60000} minute maximum transfer duration`
    failure = Object.assign(
      new Error(
        `${label}: ${reason} (${formatProgress(stats)}). Check the branch WAN/VPN/SMB connection and retry Import Agent.`
      ),
      {
        code: idle ? 'AGENT_TRANSFER_IDLE_TIMEOUT' : 'AGENT_TRANSFER_MAX_TIMEOUT',
        phase: label,
        ...stats
      }
    )
  ...
  const resetIdle = () => {
    ...
    idleTimer = setTimeout(() => expire(true), idleTimeoutMs)
  }
  const advance = (count) => {
    if (count > 0 && !failure) {
      bytes += count
  ...
  try {
    ...
    const detail =
      error.code === 'ABORT_ERR' || error.name === 'AbortError'
        ...
        : error.message
    throw Object.assign(
      new Error(`${label}: ${detail} (${formatProgress(statistics())})`, { cause: error }),
      {
        code: error.code || 'AGENT_TRANSFER_FAILED',
        phase: label,
        ...statistics()
      }
    )
...
async function hashFile(file, options = {}) {
  return runTransfer(
    async ({ signal, advance }) => {
      ...
      const input = (options.createReadStream || fs.createReadStream)(file, {
        highWaterMark: TRANSFER_BUFFER_BYTES
      })
      await pipeline(
        input,
        new Writable({
...
async function copyFile(source, destination, options = {}) {
  return runTransfer(
    async ({ signal, advance, idleTimeoutMs }) => {
      ...
      const output = (options.createWriteStream || fs.createWriteStream)(destination, {
        flags: 'wx',
        highWaterMark: TRANSFER_BUFFER_BYTES
      })
      let lastWritten = 0
