...
function registerIpcHandlers({ database, remoteService, vpnService, terminalService, updateService, storeUpdateService, storeInstallService, getWindow }) {
    ...
    const secure = (handler) => async (event, ...args) => {
        trusted(event);
        if (!sessions.has(event.sender.id))
    ...
    ipcMain.handle('auth:status', open((event) => ({
    ...
    ipcMain.handle('monitor:snapshot', secure(() => {
    ...
    ipcMain.handle('credentials:assign-device', secure((event, payload) => database.setDeviceCredential(payload?.deviceId, payload?.credentialId ?? null, sessions.get(event.sender.id).username)));
    ipcMain.handle('credentials:assign-type', secure((event, payload) => database.setTypeCredential(payload?.deviceType, payload?.credentialId ?? null, sessions.get(event.sender.id).username)));
    ...
    ipcMain.handle('remote:palette', open((_event, palette) => {
    ...
    ipcMain.handle('terminal:close', secure((event, sessionId) => {
    ...
    const listServerFiles = async (dirPath) => {
        ...
        try {
            ...
            return { directory: targetDir, parent: parent !== targetDir ? parent : null, files };
        }
        catch (error) {
            throw new Error(`Cannot list ${targetDir}: ${error.message}`);
        }
    };
    ...
    ipcMain.handle('store-update:import-agent', secure(async (event, payload) => {
        ...
        database.audit(sessions.get(event.sender.id).username, 'AGENT_IMPORT', result.name || result.host || 'checkout', result.ok
    ...
    ipcMain.handle('store-update:import-agent-all', secure(async (event, payload) => {
        ...
        if (checkouts.length > 2000)
            throw new Error('At most 2000 checkouts can be imported in one run');
    ...
    ipcMain.handle('store-update:test-access', secure(async (event, payload) => {
        ...
        if (!host)
            throw new Error('Enter the hostname or IP of a checkout to test against');
        const credentials = {
    ...
    ipcMain.handle('store-update:deploy', secure(async (event, payload) => {
        ...
        database.audit(sessions.get(event.sender.id).username, 'STORE_DEPLOY_ONE', `${payload?.checkout?.name || payload?.checkout?.hostname || 'checkout'} — ${result.ok ? 'OK' : 'FAILED'}`, `${result.error || `Deployed ${result.bytes ?? 0} bytes in ${result.durationMs} ms`}${result.backup ? `; backup: ${result.backup}` : ''}`);
        return result;
    }));
    ipcMain.handle('store-update:deploy-all', secure(async (event, payload) => {
        ...
        database.audit(sessions.get(event.sender.id).username, 'STORE_DEPLOY_ALL', `${summary.ok}/${summary.total} checkout(s) updated`, `File: ${String(payload?.source || '')
            .split(/[\\/]/)
            .pop() || '—'} → ${String(payload?.destinationPath || '—')} (${summary.durationMs} ms)`);
        return summary;
    }));
    ...
    ipcMain.handle('store-update:install-one', secure(async (event, payload) => {
        ...
        database.audit(sessions.get(event.sender.id).username, 'STORE_INSTALL_ONE', `${payload?.checkout?.name || payload?.checkout?.hostname || 'checkout'} — ${result.ok ? 'OK' : result.cancelled ? 'STOPPED' : 'FAILED'}`, result.ok
    ...
    ipcMain.handle('store-update:install-all', secure(async (event, payload) => {
        const checkouts = Array.isArray(payload?.checkouts) ? payload.checkouts : [];
        if (checkouts.length > 2000)
            throw new Error('At most 2000 checkouts can be updated in one run');
        const summary = await storeInstallService.installAll(checkouts, {
            destinationPath: String(payload?.destinationPath || ''),
            runId: payload?.runId
        });
        database.audit(sessions.get(event.sender.id).username, 'STORE_INSTALL_ALL', `${summary.ok}/${summary.total} checkout(s) updated`, `Store Commerce installer install (${summary.durationMs} ms)`);
        return summary;
    }));
    ...
    ipcMain.handle('app:info', secure(() => ({
    ...
    const companionServer = createCompanionServer({
        ...
        // Store-update (import agent, version checks, deploy, install) for phone via server (v3.10.2)
        storeUpdateService,
        storeInstallService
    });
    ipcMain.handle('companion:state', secure(() => companionServer.state()));
    ipcMain.handle('companion:set', secure((_event, patch) => companionServer.configure(patch)));
    ipcMain.handle('companion:rotate-token', secure(() => companionServer.rotateToken()));
