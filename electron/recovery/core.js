/**
 * Credential recovery core (shared).
 * ---------------------------------
 * All of the recovery logic lives here so it can run in two hosts:
 *
 *   1. the main application in `--recovery` mode (electron/main/recovery-mode.js)
 *   2. the legacy standalone tool build (electron/recovery/index.js)
 *
 * It reads ONE small encrypted file (`credentials.dat`) that the main
 * application refreshes at every start and after every password change. No
 * database, no SQLCipher, no native modules — just Node's built-in
 * fs/path/crypto plus Electron's DPAPI-backed safeStorage, exactly like the
 ...
 *     inside the same file, so every host shares one gate
 *   - DPAPI ties decryption to the Windows user, so recovery can only ever
...
/**
 * One recovery session: loads and decrypts the credentials file, then answers
 * state queries and PIN attempts. The two hosts each create their own
 * session, so no state is shared between processes.
 */
function createRecoverySession(app) {
  ...
  function load() {
    ...
    try {
      ...
      state.error = `The saved credentials could not be decrypted (${error.message}). Recovery only works for the same Windows user who runs the application.`
    }
  }
  ...
  return { MAX_ATTEMPTS, load, publicState, verify }
}

module.exports = { createRecoverySession }
