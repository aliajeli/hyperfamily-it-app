...
const revealTextField = async (page) => {
  ...
  const trigger = page
    .getByRole('button', { name: /add branch|add device|new branch|new device|add/i })
    .first()
...
async function main() {
   ...
   * Baseline: no stray inert state on a freshly loaded page. */
  {
    ...
    record(
      '1. devices page loads with no pointer-events lock',
   ...
   * root rather than through the UI. */
  {
    ...
    const state = await inertState(page)
    record(
      '2. injected pointer-events lock is auto-released by the guard',
      state.pointerEvents !== 'none',
      JSON.stringify(state)
    )
    await context.close()
   ...
   * the user does. Verifies both focus and value. */
  {
    ...
    record('3. real mouse click + keystrokes reach a text box after a lock', ok, `value="${typed}"`)
    await context.close()
  }
   ...
   * click-based test can miss. */
  {
    ...
    record(
      '4. keyboard-only navigation can focus and type',
      landed && value.includes('keyboard-only'),
      `focused=${landed} value="${value}"`
    )
    await context.close()
  }
   ...
   * not a one-shot. */
  {
    ...
    record(
      '5. ten consecutive lock/recover cycles all release',
      failures === 0,
      `${10 - failures}/10 released`
    )
    await context.close()
  }
   ...
   * lock in place and still allow typing inside the dialog. */
  {
    ...
    record(
      '6. typing works inside an open modal (guard is not over-eager)',
      !dialogOpen || canType,
      `dialogOpen=${dialogOpen} canType=${canType}`
    )
    await context.close()
  }
   ...
   * Settings, which is tab-gated. */
  {
    ...
    record(
      '7. credentials panel accepts typing after a lock',
      value.includes('cred-test'),
      `value="${value}"`
    )
    await context.close()
  }
   ...
   * it to throw, so any surviving call fails loudly instead of silently. */
  {
    ...
    record(
      '8. no shipped bundle relies on native confirm() for deletes',
      sources <= 1,
      `bundles referencing confirm: ${sources} (fallback allows 1), runtime calls: ${calls}`
    )
    await context.close()
  }
   ...
   * note, then prove the page still accepts keystrokes. */
  {
    ...
    const dialog = await page
      ...
      .catch(() => '')
    record(
      '9. deleting a note opens the themed dialog, not a native one',
      /Delete/i.test(dialog),
      `dialog text: ${dialog.slice(0, 60)}`
    )
    ...
    const state = await inertState(page)
    record(
      '10. body stays interactive after deleting a note',
      state.pointerEvents !== 'none',
      JSON.stringify(state)
    )
    ...
    record(
      '11. the note title accepts typing after a delete',
      (await title.inputValue()) === 'note-after-delete',
      `value="${await title.inputValue()}"`
    )
    ...
    record(
      '12. the note body accepts typing after a delete',
      (await body.inputValue()).includes('body-after-delete'),
    ...
    await page.locator('button').filter({ hasText: 'Branch rollout' }).first().click({ force: true })
    ...
    const discard = await page
      ...
      .catch(() => '')
    record(
      '13. switching away from an edited note warns with the themed dialog',
      /Discard/i.test(discard),
      `dialog text: ${discard.slice(0, 60)}`
    )
    ...
    record(
      '14. body stays interactive after cancelling a dialog',
      afterCancel.pointerEvents !== 'none',
    ...
    record(
      '15. typing still works after a cancelled dialog',
      (await title.inputValue()).endsWith('!'),
      `value="${await title.inputValue()}"`
    )
    await context.close()
  }
   ...
   * scrim still intercept clicks? */
  {
    ...
    record(
      '16. an orphaned dialog overlay no longer intercepts clicks',
      intercepts === false,
    ...
    if (haveField) {
      ...
      record(
        '17. a real click reaches the text box after an orphaned overlay',
        value.includes('overlay-cleared'),
        `value="${value}"`
      )
    ...
    record(
      '18. stale aria-hidden/inert on the app root is cleared',
      stuck.hidden === 0 && stuck.inert === 0,
    ...
    if (haveField) {
      ...
      const value = await title.inputValue()
      record(
        '19. typing works again after the app root was left inert',
        value.includes('inert-cleared'),
        `value="${value}"`
      )
    ...
    record(
      `20. ${rounds} rounds of combined wreckage all recover`,
      recovered === rounds,
      `${recovered}/${rounds}`
    )
    ...
    if (haveField) {
      ...
      const value = await title.inputValue()
      record(
        '21. text box is still usable after repeated combined wreckage',
        value.includes('OK21') && value !== before21,
        `value="${value}"`
      )
   ...
   * must not become a new bug. */
  {
    ...
    record(
      '22. an open dialog and its overlay survive the guard',
      before > 0 && after === before && overlayVisible,
      `dialogs ${before} -> ${after}, overlay visible=${overlayVisible}`
    )
    await context.close()
  }
