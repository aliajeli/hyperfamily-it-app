...
class TestVault {
  constructor(directory) {
    this.keyFile = path.join(directory, '.test-db-key')
  }
  encrypt(value) {
    return `test:${Buffer.from(String(value)).toString('base64')}`
  }
  decrypt(value) {
    return Buffer.from(String(value).replace(/^test:/, ''), 'base64').toString()
  }
  hasDatabaseKey() {
...
test('updates username and password without recreating the default Admin account', () => {
  ...
  try {
    ...
    const updated = database.updateCredentials(1, {
      currentPassword: 'Admin',
      newUsername: 'OperationsAdmin',
      newPassword: 'SecurePass123!'
    })
    assert.deepEqual(updated, { id: 1, username: 'OperationsAdmin' })
    assert.equal(database.authenticate('Admin', 'Admin'), null)
    assert.deepEqual(database.authenticate('operationsadmin', 'SecurePass123!'), {
      id: 1,
      username: 'OperationsAdmin'
    })

    const renamed = database.updateCredentials(1, {
      currentPassword: 'SecurePass123!',
      newUsername: 'BranchAdmin'
    })
    assert.deepEqual(renamed, { id: 1, username: 'BranchAdmin' })
    assert.deepEqual(database.authenticate('BranchAdmin', 'SecurePass123!'), {
      id: 1,
      username: 'BranchAdmin'
    })

    database.updateCredentials(1, {
      currentPassword: 'SecurePass123!',
      newUsername: 'BranchAdmin',
      newPassword: 'AnotherSecurePass!'
    })
    assert.deepEqual(database.authenticate('BranchAdmin', 'AnotherSecurePass!'), {
      id: 1,
      username: 'BranchAdmin'
    })
    assert.throws(
      () => database.updateCredentials(1, { currentPassword: 'wrong', newUsername: 'OtherAdmin' }),
...
test('persists branch, device, ping snapshot, and encrypted settings', () => {
  ...
  try {
    ...
    const device = database.saveDevice({
      branch_id: branch.id,
      device_type: 'Router',
      name: 'Gateway',
      ip: '10.0.0.1',
    ...
    database.saveSettings({
      vpn_pass: 'sensitive-secret',
      dashboard_branch_mode: 'always_compact',
      dashboard_branch_details_view: 'side_panel'
    })
...
test('persists device-specific fields, edits devices, and replaces managed switch ports', () => {
  const { database, cleanup } = fixture()
  try {
    const branch = database.saveBranch({
      name: 'Port Test Branch',
      code: 'PORT-01',
      warehouse_code: 'WH-PORT-01'
    })
    ...
    assert.deepEqual(
      updated.switch_ports.map((port) => [port.port_number, port.vlan, port.status]),
      [
        [1, '110', 'up'],
        [3, '30', 'disabled']
      ]
    ...
    const scale = database.saveDevice({
      branch_id: branch.id,
      device_type: 'Scale',
      name: 'Deli Scale',
      model: 'Mettler',
...
test('enforces the 48-port Switch boundary in service validation and SQLite triggers', () => {
  ...
  try {
    const branch = database.saveBranch({
      ...
      code: 'LIMIT-01',
      warehouse_code: 'WH-LIMIT-01'
    })
    const ports = Array.from({ length: 48 }, (_, index) => ({
      port_number: index + 1,
      vlan: String(100 + index),
      status: 'up',
      ip: '',
      details: `Port ${index + 1}`
    }))
    const fullSwitch = database.saveDevice({
      branch_id: branch.id,
      device_type: 'Switch',
      name: 'Full Switch',
      ip: '10.22.1.2',
    ...
    assert.throws(
      () => database.saveDevice({ ...fullSwitch, switch_ports: [{ port_number: 49, status: 'up' }] }),
      /from 1 through 48/
    )
    assert.throws(
      () =>
        database.saveDevice({
          ...fullSwitch,
          switch_ports: [
            { port_number: 1, status: 'up' },
            { port_number: 1, status: 'down' }
          ]
        }),
      /duplicated|unique/
    )
    ...
    `
          ...
          .run(otherSwitch.id),
      /from 1 through 48|CHECK constraint failed/
    )
    assert.throws(
      () =>
        database.db
    ...
    `
          ...
          .run(fullSwitch.id, otherSwitch.id),
      /at most 48 ports/
    )
  } finally {
    cleanup()
...
test('upgrades older databases with Warehouse Code, scale serial numbers, and managed switch ports', () => {
  ...
  try {
    ...
    legacy.exec(
      'DROP TABLE switch_ports; ALTER TABLE devices DROP COLUMN serial_number; DROP INDEX idx_branches_warehouse_code_unique; DROP INDEX idx_devices_one_router_per_branch; ALTER TABLE branches DROP COLUMN warehouse_code; PRAGMA user_version = 1;'
    )
  } finally {
    legacy.close()
  }
  ...
  try {
    ...
    assert.equal(
      database.db
        .prepare("SELECT COUNT(*) FROM sqlite_master WHERE type = 'table' AND name = 'switch_ports'")
        .pluck()
        .get(),
      1
    )
    assert.equal(
      database.db
        .prepare(
          "SELECT COUNT(*) FROM sqlite_master WHERE type = 'index' AND name = 'idx_devices_one_router_per_branch'"
        )
        .pluck()
        .get(),
      1
...
test('exports serial numbers, dashboard choices, and managed Switch ports to Excel', async () => {
  ...
  try {
    ...
    database.saveDevice({
      ...
      is_dashboard_visible: true,
      switch_ports: [
        { port_number: 7, vlan: '170', status: 'up', ip: '10.30.170.1', details: 'Export uplink' }
      ]
    })
    database.saveDevice({
      branch_id: branch.id,
    ...
    assert.match(
      sheet.getColumn(headers.indexOf('Switch Ports')).values.join(' '),
      /Port 7 · VLAN 170 · up · 10\.30\.170\.1 · Export uplink/
    )
    assert.equal(sheet.getColumn(headers.indexOf('Dashboard')).values.includes('Shown'), true)

    const filtered = await exportInventory(
      database,
      { branch: branch.id, type: 'Switch', query: 'Export uplink' },
      filteredPath
    )
...
test('requires Device Name and enforces one Router per branch during add and edit', () => {
  ...
  try {
    ...
    assert.throws(
      () => database.saveBranch({ name: 'Duplicate Warehouse', code: 'RTR-02', warehouse_code: 'wh-rtr-01' }),
      /UNIQUE constraint failed/
    )
    assert.throws(
      () => database.saveDevice({ branch_id: branch.id, device_type: 'Scale', ip: '10.41.1.20' }),
      /Device Name is required/
    )
    const router = database.saveDevice({
      branch_id: branch.id,
      device_type: 'Router',
      name: 'Primary Router',
      ip: '10.41.1.1'
...
test('creates the official type-specific template and atomically imports branches, devices, and Switch ports', async () => {
  ...
  try {
    ...
    assert.deepEqual(workbook.getWorksheet('Branches').getRow(1).values.slice(1), [
      'Branch Name',
      'Branch Code',
      'Warehouse Code',
      'Link1',
    ...
    for (const type of expectedSheets.slice(2)) {
      ...
      if (type === 'Switch') {
        assert.deepEqual(
          headers.slice(0, expectedEquipmentHeaders.Switch.length),
          expectedEquipmentHeaders.Switch
        )
    ...
    assert.equal(
      switchSheet.getRow(2).getCell(switchSheet.getRow(1).values.indexOf('Port 48 Number')).dataValidation
        .formulae[1],
      48
    )

    setValues(workbook.getWorksheet('Branches'), 2, {
      'Branch Name': 'Imported Branch',
      'Branch Code': 'IMP-01',
      'Warehouse Code': 'WH-IMP-01',
      Link1: 'Fiber',
      'IP Link1': '10.50.1.1',
      Link2: 'LTE',
      'IP Link2': '10.50.1.2',
    ...
    setValues(switchSheet, 2, {
      'Branch Code': 'IMP-01',
      'Device Name': 'Imported Core',
      Model: 'CBS350',
      Location: 'Network Room',
      IP: '10.50.1.2',
      'Connection Type': 'Fiber',
      'Connection Port': 'Gi1/0/24',
      'Asset Code': 'SW-001',
      Dashboard: 'Show',
      'Port 1 Number': 1,
      'Port 1 VLAN': '10',
      'Port 1 Status': 'Up',
      'Port 1 IP': '10.50.10.1',
      'Port 1 Details': 'Server uplink',
      'Port 2 Number': 2,
      'Port 2 VLAN': '20',
      'Port 2 Status': 'Disabled',
      'Port 2 Details': 'Reserved'
    })
    setValues(workbook.getWorksheet('Scale'), 2, {
      'Branch Code': 'IMP-01',
      'Device Name': 'Deli Scale',
      Model: 'Mettler',
      Location: 'Deli',
      IP: '10.50.1.40',
      'Serial Number': 'SN-IMPORT-1',
      'Asset Code': 'SC-001',
      Dashboard: 'Hide'
    })
    ...
    assert.equal(
      database.listDevices().find((device) => device.device_type === 'Switch').switch_ports.length,
      2
    )
    assert.equal(
      database.listDevices().find((device) => device.device_type === 'Scale').serial_number,
      'SN-IMPORT-1'
    )
    ...
    assert.equal(
      database.listDevices().find((device) => device.device_type === 'Router').name,
      'Updated Gateway'
    )
...
test('rejects an invalid Excel import without partially saving valid rows', async () => {
  ...
  try {
    ...
    workbook.getWorksheet('Router').getRow(2).values = [
      'ATM-01',
      'Router One',
      '',
      '10.60.1.1',
    ...
    workbook.getWorksheet('Router').getRow(3).values = []
    workbook.getWorksheet('Switch').getRow(2).values = [
      'ATM-01',
      'Out of Range Switch',
      '',
      '',
...
test('credential mappings cascade when a credential is deleted', () => {
  ...
  try {
    ...
    const credential = database.saveCredential({
      name: 'Admin set',
...
test('migrates a plaintext database only after validating the encrypted copy', () => {
  ...
  try {
    ...
    try {
      assert.equal(backup.prepare('SELECT value FROM legacy_probe').pluck().get(), 'legacy-value')
    } finally {
      backup.close()
    }
    ...
    try {
      assert.throws(
        () => unkeyed.prepare('SELECT value FROM legacy_probe').get(),
        /file is not a database|encrypted/i
      )
    } finally {
      unkeyed.close()
    }
...
test('keeps plaintext intact after a failed migration and retries with an existing key', () => {
  ...
  try {
    ...
    try {
      assert.equal(database.db.prepare('SELECT value FROM legacy_probe').pluck().get(), 'retry-succeeded')
    } finally {
      database.close()
    }
  } finally {
...
test('deleteAllBranchesAndDevices wipes the whole directory', () => {
  ...
  try {
    ...
    assert.equal(database.listDevices().length, 0)
    assert.equal(
      database.listAudit(10).some((entry) => entry.action === 'DIRECTORY_CLEAR'),
      true
    )
...
test('recovery PIN gates reveal with a shared 5-attempt lockout', () => {
  ...
  try {
    ...
    database.writeRecoveryFile({
      v: 2,
      username: 'Admin',
      password: 'Admin',
      pinHash: file.pinHash,
      attempts: 0,
      lockedUntil: 0
    })
    outcome = database.verifyRecoveryPin('4821')
