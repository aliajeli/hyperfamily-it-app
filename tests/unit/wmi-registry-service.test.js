...
const {
  ...
  runWmiQuery,
  WMI_REGISTRY_SCRIPT
} = require('../../electron/services/wmi-registry.service')
...
test('WMI distinguishes an empty inventory from malformed/failed output', async () => {
  ...
  for (const raw of ['', 'garbage', 'null', '{}', '[null]', '[{}]']) {
    await assert.rejects(
      listProgramsViaWmi('CO-01', { run: async () => raw }),
      /invalid installed-programs response/
    )
  }
  await assert.rejects(
    listProgramsViaWmi('host;bad', {
      run: async () => {
        assert.fail()
      }
    }),
...
test('process errors and timeouts never leak raw PowerShell diagnostics', async () => {
  for (const killed of [true, false]) {
    await assert.rejects(
      runWmiQuery({ password: 'secret-fixture' }, 10, (_command, _args, _options, callback) => {
        setImmediate(() =>
          callback(Object.assign(new Error('secret-fixture'), { killed }), '', 'secret-fixture')
        )
        return {
          stdin: new Writable({
...
test('WMI script is read-only and does not require WinRM or RemoteRegistry', () => {
  ...
  assert.doesNotMatch(
    WMI_REGISTRY_SCRIPT,
    /Win32_Product|Invoke-Command|Start-Service|Set-Service|SetStringValue/
  )
})
...
test('WMI script parses in Windows PowerShell', { skip: process.platform !== 'win32' }, async () => {
  ...
  const result = await defaultRunPs(
    `$tokens = $null; $errors = $null; [void][System.Management.Automation.Language.Parser]::ParseInput(${psLiteral(WMI_REGISTRY_SCRIPT)}, [ref]$tokens, [ref]$errors); if ($errors.Count) { throw ($errors | Out-String) }; 'ok'`
  )
