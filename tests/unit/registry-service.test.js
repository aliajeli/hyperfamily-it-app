...
const {
  ...
  parseRegQuery,
  pickProgram,
  pickHyperCommerceExtension
} = require('../../electron/services/registry.service')
...
test('listRemotePrograms queries both registry views of the remote host', async () => {
  ...
  const rows = await listRemotePrograms('CO-01', {
    exec: async (args) => {
      seen.push(args[1])
      return { ok: true, stdout: SAMPLE, stderr: '' }
    }
  })
...
test('listRemotePrograms targets the local machine when no host is given', async () => {
  const seen = []
  await listRemotePrograms('', {
    exec: async (args) => {
      seen.push(args[1])
      return { ok: true, stdout: SAMPLE }
    }
  })
...
test('listRemotePrograms explains access denied and a stopped Remote Registry', async () => {
  ...
  await assert.rejects(
    listRemotePrograms('CO-01', {
      exec: async () => ({ ok: false, stderr: 'ERROR: The RPC server is unavailable.' })
    }),
    /Remote Registry/
  )
  await assert.rejects(
...
test('credentialsFrom reads the Target access settings', () => {
  const credentials = SmbSessionManager.credentialsFrom({
    target_domain: 'okcs ',
    target_admin_user: ' administrator',
    target_admin_password: 'pw'
  })
...
test('withHost runs the task unchanged when no credentials are configured', async () => {
  const manager = new SmbSessionManager({
    platform: 'win32',
    exec: async () => {
      throw new Error('net use must not run')
    }
  })
  ...
  const manager = new SmbSessionManager({
    ...
    exec: async (_command, args) =>
      args.includes('/delete')
        ? { ok: true }
        : { ok: false, stderr: 'System error 1326: The user name or password is incorrect.' }
  })
  await assert.rejects(
    manager.withHost(
      'CO-01',
      { domain: 'okcs', username: 'administrator', password: 'bad' },
      async () => 'never'
    ),
...
test('withHost releases the session even when the task throws', async () => {
  ...
  await assert.rejects(
    manager.withHost('CO-01', { domain: 'okcs', username: 'a', password: 'b' }, async () => {
...
test('backup hive queries the correct paths and unloads/cleans up on failure', async () => {
  ...
  try {
    for (const failQuery of [false, true]) {
      ...
      assert.ok(
        queries.some((key) => /\\WOW6432Node\\Microsoft\\Windows\\CurrentVersion\\Uninstall$/.test(key))
      )
