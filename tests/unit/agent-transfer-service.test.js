...
const {
  ...
  TRANSFER_IDLE_MS,
  TRANSFER_MAX_MS
} = require('../../electron/services/agent-transfer.service')
...
test('slow but progressing SHA-256 reads can run longer than an idle window', async () => {
  ...
  const actual = await hashFile('slow-remote-file', {
    ...
    maxDurationMs: 5000,
    totalBytes: 55,
    createReadStream: (_file, options) => {
...
test('network errors retain their code and gain a useful phase label', async () => {
  ...
  await assert.rejects(
    hashFile('file', { label: 'Verifying staged agent on CO-03', createReadStream: () => input }),
    (error) => {
