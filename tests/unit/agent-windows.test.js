...
test(
  'native EXE is small, standalone, and reads Unicode plus both registry views',
  ...
  async () => {
    ...
    const versionInfo = await exec('powershell.exe', [
      '-NoProfile',
      '-NonInteractive',
      '-Command',
    ...
    try {
      ...
      for (const view of [64, 32]) {
        ...
        assert.equal(
          row.installLocation.toLowerCase(),
          `${process.env.SystemRoot}\\HyperFamily`.toLowerCase()
        )
      }
      console.log(`Native agent: ${binary.size} bytes; imports: ${binary.imports.join(', ')}`)
    } finally {
      for (const view of [64, 32])
        await exec('reg.exe', ['delete', key, '/f', `/reg:${view}`]).catch(() => {})
    }
  }
)

test(
  'Windows EXE: self-test, real LocalSystem install, heartbeat, hash skip and stop detection',
  ...
  async () => {
    ...
    control.runPs = (script, timeout) =>
      runPs(script.replace(psLiteral('\\\\localhost\\C$\\Agent'), psLiteral('C:\\Agent')), timeout)
    try {
      ...
      assert.ok(
        !data.programs.some((row) => row.key === 'SelfTest'),
        'Self-test fixture must never appear in live inventory'
      )
      await delay(17000)
